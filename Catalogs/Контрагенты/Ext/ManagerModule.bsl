...
Процедура ЗарегистрироватьКонтрагентовДляЗаполнениюСНО(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|	И Контрагенты.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.ТипыСистемНалогообложенияККТ.ПустаяСсылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если КонтрагентОбъект.УдалитьСистемаНалогообложения = Перечисления.УдалитьСистемыНалогообложения.Общая Тогда
				КонтрагентОбъект.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.ОСН;
			ИначеЕсли КонтрагентОбъект.УдалитьСистемаНалогообложения = Перечисления.УдалитьСистемыНалогообложения.Упрощенная Тогда
				КонтрагентОбъект.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоход;
			КонецЕсли;
