...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(СистемаНалогообложения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""%1"" не заполнено.'"), "Система налогообложения"),,, "СистемаНалогообложения");
		Отказ = Истина;
	ИначеЕсли СистемаНалогообложения = "УСН" И НЕ ЗначениеЗаполнено(УСНОбъектНалогообложения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""%1"" не заполнено.'"), "Объект налогообложения"),,, "УСНОбъектНалогообложения");
		Отказ = Истина;		
	КонецЕсли;	
...
Функция СНООрганизации(Организация)
	...
	Если СНООрганизации <> Неопределено Тогда
		...
		ИначеЕсли СНООрганизации.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоходРасход Тогда
			Результат.СНО 					= "УСН";
			Результат.ОбъектНалогообложения = "Доход минус расход";
...
Процедура УправлениеЭлементамиСНО()
	...
	ВидимостьПлательщикНДС = СистемаНалогообложения = "ОСН" ИЛИ СистемаНалогообложения = "ЕСН";
	Элементы.ОсвобожденОтНДС.Видимость 				= ВидимостьПлательщикНДС;
