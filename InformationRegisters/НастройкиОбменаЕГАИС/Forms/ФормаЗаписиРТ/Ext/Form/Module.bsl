...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОбменНаСервере.Видимость = 
		...
		ИЛИ ОбщегоНазначенияКлиентСервер.КлиентПодключенЧерезВебСервер();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если не МенеджерЗаписи.Выбран() ИЛИ МенеджерЗаписи.ФорматОбмена = ФорматОбмена Тогда
...
Процедура ЗагрузитьОрганизациюПоИНН(Команда)
	...
	ИнтеграцияЕГАИСКлиент.НачатьФормированиеИсходящегоЗапроса(
		...
		ВидДокумента,
		ПараметрыЗапросаКлассификатора(Организация, ВидДокумента));
...
Функция ПараметрыЗапросаКлассификатора(Организация, ВидДокумента)
	
	ВходныеПараметры = ИнтеграцияЕГАИСКлиентСервер.ПараметрыИсходящегоЗапроса(ВидДокумента);
	ВходныеПараметры.ИНН = Организация.ИНН;
...
&НаКлиенте
Процедура ФормированиеЗапросаКлассификатора_Завершение(ДанныеЗапроса, ДополнительныеПараметры) Экспорт
	
	Если НЕ ДанныеЗапроса.Результат Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ДанныеЗапроса.ОписаниеОшибки);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДанные()
	...
	Таблица.Колонки.Добавить("Магазин", Новый ОписаниеТипов("СправочникСсылка.Магазины"));
	...
	НоваяСтрока.Магазин = Запись.Магазин;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(Таблица.Организация КАК Справочник.Организации) КАК Организация,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИсходныеДанные.Организация.ИНН КАК ИНН,
	|	ИсходныеДанные.Организация.КПП КАК КПП,
	|	ЕСТЬNULL(КлассификаторОрганизацийЕГАИС.ИНН, """") КАК ИНН_ЕГАИС,
	|	ЕСТЬNULL(КлассификаторОрганизацийЕГАИС.КПП, """") КАК КПП_ЕГАИС,
	...
	|		ПО ИсходныеДанные.КодВФСРАР = КлассификаторОрганизацийЕГАИС.Код";
	...
	Если Выборка.Следующий() Тогда
		...
		Элементы.ИНННеСоответствуетОрганизацииЕГАИС.Видимость = (Выборка.ИНН <> Выборка.ИНН_ЕГАИС) И НЕ ПустаяСтрока(Запись.ИдентификаторФСРАР);
		Элементы.КППНеСоответствуетОрганизацииЕГАИС.Видимость = (Выборка.КПП <> Выборка.КПП_ЕГАИС) И НЕ ПустаяСтрока(Запись.ИдентификаторФСРАР);
	...
	Если НЕ ПустаяСтрока(Запись.ИдентификаторФСРАР) Тогда
		ИспользуемыйФормат = ИспользуемыйФорматОбмена(Запись.ИдентификаторФСРАР);
		Если не ИспользуемыйФормат = Неопределено Тогда
			ФорматОбмена = ИспользуемыйФормат;
		КонецЕсли;
	Иначе
...
Процедура ПриИзмененииВариантаПодключения()
	...
	ИначеЕсли ОбменНаСервере = 1 Тогда
		Подсказка = НСтр("ru = 'Подключение к УТМ будет осуществляться с компьютера, на котором расположена база данных 1С:Предприятия'");
	КонецЕсли;
	
	Элементы.ОбменНаСервере.Подсказка = Подсказка;
	
	Если ОбменНаСервере = 1 И НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() ИЛИ НЕ ЗначениеЗаполнено(РабочееМесто) Тогда
