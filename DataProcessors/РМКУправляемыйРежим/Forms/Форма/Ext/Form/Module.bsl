...
Процедура ОбработатьДанныеПоКодуСервер(СтруктураРезультат) Экспорт
	...
	Если СтрокаРезультата.Свойство("НеобходимостьВводаАкцизнойМаркиДляТабака") Тогда
		СтруктураРезультат.Вставить("НеобходимостьВводаАкцизнойМаркиДляТабака", СтрокаРезультата.НеобходимостьВводаАкцизнойМарки);
	КонецЕсли;
...
Функция ДобавитьНайденныеПозицииТоваров(СтруктураПараметров) Экспорт 
	...
	Если ДобавленаСтрока Тогда
		...
		СтруктураПараметровЗаполненияОрганизации = Новый Структура("Магазин, РабочееМесто", Объект.Магазин, Объект.РабочееМесто);
		...
		СтруктураПроверкиАМ.Вставить("ВестиУчетТабачнойПродукцииМОТП", ЭтотОбъект.ВестиУчетТабачнойПродукцииМОТП);
...
Процедура ПослеОбработкиШтрихкодов(РезультатОбработкиШтрихкода,КэшированныеЗначения)
	...
	Если РезультатОбработкиШтрихкода.Свойство("ДобавленныеСтроки") Тогда 
		Для Каждого Строка Из РезультатОбработкиШтрихкода.ДобавленныеСтроки Цикл 
			...
			СтруктураПараметров.ШтрихкодУпаковкиЕГАИС = Справочники.ШтрихкодыУпаковокТоваров.ПолучитьАкцизнуюМаркуПоКоду(Строка.Штрихкод);
			СтруктураПараметров.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар;
			
			ДобавитьНайденныеПозицииТоваров(СтруктураПараметров);
	...
	Иначе
		...
		Если ТипЗнч(Результат) = Тип("Строка") Тогда
			...
			Иначе
				...
				СтрокаТабличнойЧасти.Упаковка = ПредопределенноеЗначение("Справочник.УпаковкиНоменклатуры.ПустаяСсылка");
				СтрокаТабличнойЧасти.КоличествоУпаковок = 1;
				СтрокаТабличнойЧасти.Штрихкод = Результат;
...
Процедура ПриЗакрытииФормыПроверкиИПодбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоАдресВременногоХранилища(Результат) Тогда 
		ДанныеДляОбработки = Новый Структура("АдресВХранилище", Результат);
	ИначеЕсли Результат = Неопределено Тогда
		ДанныеДляОбработки = Результат;
	Иначе 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучитьТаблицуСистемНалогообложений();
	...
	ВидимостьКолонкиМаркировки = УчетАлкогольнойПродукции Или ВестиУчетТабачнойПродукцииМОТП
								Или ИспользуетсяОбувнаяМаркировка;
...
Процедура ВыполнитьПО14Продавец()
	...
	Иначе
		...
		Режим = РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс;
		ОткрытьФорму("Справочник.ФизическиеЛица.Форма.ФормаВыбораВРМК", ПараметрыФормы, УникальныйИдентификатор,,,, ОбработчикОповещения, Режим);
	КонецЕсли;
...
Процедура ЗаполнитьЧекПоВсемДанным(ОбъектЧекККМ)
	...
	Если ТабличноеПолеЧеков.Количество() <> 0 Тогда
		ОбъектЧекККМ.ДоговорПлатежногоАгента 	= ТабличноеПолеЧеков[0].ДоговорПлатежногоАгента;
...
Процедура ЗаполнитьНаОснованииЧекаККМ(ЧекККМ)
	...
	ЗаполнитьЗначенияСвойств(Объект, 
							 ЧекККМ, 
							 "ВладелецДисконтнойКарты, ДисконтнаяКарта, Магазин, Продавец, ЗаказПокупателя, ДоговорПлатежногоАгента, ОперацияСДенежнымиСредствами, Контрагент", 
...
Процедура ДозаполнитьТаблицыОбъекта()
	...
	ОбработкаТабличнойЧастиТоварыСервер.ЗаполнитьСтавкуНДСВТЧСервер(Объект.Товары, 
		ОбработкаТабличнойЧастиТоварыКлиентСервер.СтруктураПараметровСтавкиНДСВТЧ(Объект));
	ОбработкаТабличнойЧастиТоварыСервер.ЗаполнитьНДСПоСтрочно(Объект.Товары);
	ОбработкаТабличнойЧастиТоварыСервер.ПересчитатьСуммуНДСВТЧСервер(Объект.Товары, 
...
Процедура ПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации, ОплатаТолькоНаличными)
	...
	Если НЕ Отказ Тогда
		Если НЕ Объект.ОперацияСДенежнымиСредствами
			И (ИспользуетсяРегистрацияРозничныхПродажВЕГАИС Или ВестиУчетТабачнойПродукцииМОТП Или ИспользуетсяОбувнаяМаркировка) Тогда
...
Процедура ЗаполнитьТаблицуЧеков()
	...
	Если Объект.ОперацияСДенежнымиСредствами 
		...
		СтрокаТаблицыЧеков.ВидНалога = ПодключаемоеОборудованиеРТ.ПолучитьСистемуНалогообложения(ТекущаяДатаСеанса(),
																								 СтрокаТаблицыЧеков.Организация,
																								 КассаККМСтрокиТаблицыЧеков.Магазин,
	...
	Иначе
		...
		|	ТаблицаВЗапросе.ДоговорПлатежногоАгента КАК ДоговорПлатежногоАгента
		|ПОМЕСТИТЬ ТаблицаВЗапросе
		|ИЗ
		...
		|	ТаблицаВзапросе.ДоговорПлатежногоАгента
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ТаблицаЧеков.ДоговорПлатежногоАгента КАК ДоговорПлатежногоАгента,
...
Функция ЗаписатьЧекККМПоОрганизации(ИдентификаторСтрокиТПЧеков, ПерваяОрганизация, ПараметрыОперации, ПараметрыИнформации)
	...
	ОбъектЧекККМ.ДоговорПлатежногоАгента 	= СтрокаТПЧеков.ДоговорПлатежногоАгента;
...
Процедура ПечатьЧека_ПослеОшибкиПечатиЧека(ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЕстьАлкогольнаяПродукцияЕГАИС Тогда
		...
		ИнтеграцияЕГАИССлужебныйКлиент.ОбработатьОчередьПередачиДанных(
			Новый ОписаниеОповещения("ПриОтменеОперации", ИнтеграцияЕГАИСРТКлиент, ДополнительныеПараметры.ЧекККМСсылка),
...
Процедура ВвестиАкцизнуюМарку(СтрокаТовара, ТекущееПоле = "КоличествоУпаковок", СтруктураПараметровКлиента = Неопределено)
	...
	ТабачнаяПродукция = ОбщегоНазначенияРТВызовСервера.ЗначениеРеквизитаОбъекта(СтрокаТовара.Номенклатура, "ТабачнаяПродукция");
	Если ТабачнаяПродукция Тогда 
		ВвестиТабачнуюПродукцию(ТекущееПоле, СтрокаТовара, СтруктураПараметровКлиента);
	Иначе
		...
		Если ОрганизацииЕГАИС.Количество() > 1 Тогда 
			ТекущаяОрганизацияЕГАИС = ОпределитьТекущуюОрганизациюЕГАИС(СтрокаТовара.Организация, СтрокаТовара.Склад);
			ПараметрыСканированияАкцизнойМарки.Вставить("ОрганизацияЕГАИС", ТекущаяОрганизацияЕГАИС);
		Иначе
			ПараметрыСканированияАкцизнойМарки.Вставить("ОрганизацияЕГАИС", ОрганизацияЕГАИС);
		КонецЕсли;
...
Функция ЗаполнитьМаркиНаСервере(АдресВХранилище, ДополнительныеПараметры, Организация, Номенклатура, Характеристика)
	...
	Если ТипЗнч(ТаблицаМарок) = Тип("Структура") Тогда 
		...
		НазваниеКолонкиШтрихкода = "ЗначениеШтрихкода";
	Иначе 
	...
	Для Каждого СтрокаМарки Из ТаблицаМарок Цикл
		ДанныеШтрихкодаУпаковки = ПолучитьДанныеШтрихкодаУпаковки(Организация, 
																	Объект.Магазин, 
																	ВРег(СтрокаМарки[НазваниеКолонкиШтрихкода]),
																	ДополнительныеПараметры.ТекущийКлючСвязи,
																	Номенклатура,
																	Характеристика); 
		СтрокаАкцизнаяМарка = Объект.АкцизныеМарки.Добавить();
...
&НаКлиенте
Процедура ВвестиТабачнуюПродукцию(ТекущееПоле, ТекущиеДанные, СтруктураПараметровКлиента)
	...
	Иначе
		...
		ДополнительныеПараметры.Вставить("ИдентификаторСтроки",        ИдентификаторСтроки);
		ДополнительныеПараметры.Вставить("ТекущийКлючСвязи",           ТекущаяСтрока.КлючСвязи);
		
		ОбработчикОповещения = Новый ОписаниеОповещения("ПриЗакрытииФормыПроверкиИПодбора", ЭтотОбъект, ДополнительныеПараметры);
		ОткрытьФорму(
...
Функция ПараметрыОткрытияФормыПроверкиИПодбора()
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("СтруктураПараметровКлиента", Новый Структура);
	ДополнительныеПараметры.Вставить("ИдентификаторСтроки", ИдентификаторСтроки);
	ДополнительныеПараметры.Вставить("ТекущийКлючСвязи", ТекущаяСтрока.КлючСвязи);
	...
	ПараметрыОткрытия.ИмяРеквизитаСклад             = "Объект";
	ПараметрыОткрытия.ИмяРеквизитаОрганизация       = "Объект";
	...
	ПараметрыОткрытия.ОписаниеОповещенияПриЗакрытии = Новый ОписаниеОповещения("ПриЗакрытииФормыПроверкиИПодбора", ЭтотОбъект, ДополнительныеПараметры);
...
Функция АдресДанныхПроверкиТабачнойПродукции()
	...
	ШтрихкодыТабачнойПродукции = ИнтеграцияИСРТ.ШтрихкодыСодержащиеВидыПродукции(
		СписокШтрихкодов,
	...
	ТаблицаТабачнойПродукции = ПроверкаИПодборПродукцииМОТПРТ.ТаблицаТабачнойПродукцииДокумента(ЭтотОбъект);
...
Функция НужноСворачиватьПоСтрокеПоиска(Номенклатура)
	СворачиватьПоСтрокеПоиска = Истина;
	Если Номенклатура.ОбувнаяПродукция Тогда
		СворачиватьПоСтрокеПоиска = Ложь;
	КонецЕсли;
...
&НаСервере
// Получить таблицу систем налогообложений.
//
// Параметры:
//  Нет
//
Процедура ПолучитьТаблицуСистемНалогообложений()
	...
	Запрос.Текст = 
		...
		|			&Дата,
		|			Магазин = &Магазин
		|				ИЛИ Магазин = ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка)) КАК ПрименениеСистемНалогообложенияСрезПоследних";
...
КонецПроцедуры // ПолучитьТаблицуСистемНалогообложений()

&НаСервереБезКонтекста
Функция ОпределитьТекущуюОрганизациюЕГАИС(Организация, Склад)
