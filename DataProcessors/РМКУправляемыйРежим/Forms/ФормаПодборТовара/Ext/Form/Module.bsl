...
Процедура ВыполнитьПоискВезде()
	...
	Результат = Запрос.ВыполнитьПакет();
	СписокНоменклатуры.ЗагрузитьЗначения(Результат[5].Выгрузить().ВыгрузитьКолонку("Номенклатура"));
	СписокХарактеристик.ЗагрузитьЗначения(Результат[6].Выгрузить().ВыгрузитьКолонку("Характеристика"));
...
Процедура РассчитатьОстаткиИЦены()
	...
	Если ОтображатьКоличество Тогда
		...
		ИначеЕсли ПланыОбмена.ГлавныйУзел() <> Неопределено И ПланыОбмена.ПоРабочемуМесту.ЭтоРабочееМесто() Тогда
