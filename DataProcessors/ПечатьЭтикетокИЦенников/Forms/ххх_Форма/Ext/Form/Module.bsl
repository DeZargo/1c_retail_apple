...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.АдресВХранилище) Тогда
		...
		ИначеЕсли СтруктураДанных.Свойство("СправкиБЕГАИС") Тогда
			Объект.ИсходныеДанные.Загрузить(СтруктураДанных.СправкиБЕГАИС);
			СтраницаПриОткрытии = "СтраницаТовары";
			ЭтаФорма.АвтоЗаголовок = Ложь;
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Если СтруктураДанных.СтруктураДействий.Свойство("ИспользоватьСправкиБ", ПараметрДействия) Тогда
				...
				Элементы.ТоварыСправкаБ.Видимость = ПараметрДействия;
...
Процедура ЗаполнитьТаблицуТоваровНаСервере(Сохранять = Истина)
	...
	Если Объект.ИспользоватьСправкиБ Тогда
		СтруктураНастроек.ОбязательныеПоля.Добавить("СправкаБ");
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.ТаблицаТоваров Цикл
		...
		//Zorius
		//Весовуха=Справочники.Номенклатура.НайтиПоНаименованию("Овощи,фрукты,ягоды,грибы Свежие");
		//Если НоваяСтрока.Номенклатура.ВидНоменклатуры=справочники.ххх_Справочник.ВидНоменклатурыАлкоголь.Значение или НоваяСтрока.Номенклатура.ПринадлежитЭлементу (Весовуха) тогда
		//	НоваяСтрока.страна=?(ЗначениеЗАполнено(НоваяСтрока.Номенклатура),НоваяСтрока.Номенклатура.СтранаПроисхождения,Справочники.СтраныМира.ПустаяСсылка());
		//Иначе
		//	НоваяСтрока.страна=?(ЗначениеЗАполнено(НоваяСтрока.Номенклатура),НоваяСтрока.Номенклатура.Производитель,Справочники.Контрагенты.ПустаяСсылка()); 
		//КонецЕсли;
		//Zorius	
		
		НоваяСтрока.страна=?(ЗначениеЗАполнено(СтрокаТЧ.Номенклатура),СтрокаТЧ.Номенклатура.СтранаПроисхождения,Справочники.СтраныМира.ПустаяСсылка());

		
		Если Объект.ИспользоватьСправкиБ Тогда
			НоваяСтрока.СправкаБ         = СтрокаТЧ.СправкаБ;
		КонецЕсли;
		...
		Если Сохранять Тогда
			...
			Если Объект.ИспользоватьСправкиБ Тогда
				ПараметрыОтбора.Вставить("СправкаБ", НоваяСтрока.Номенклатура);
			КонецЕсли;
			Если ИспользоватьХарактеристикиНоменклатуры Тогда
...
Процедура УстановитьВидимостьСтраниц()
	...
	СписокРолей = "АдминистрированиеАбонентаБизнесСеть,"
					+ "БазовыеПраваВнешнегоПользователя,"
					+ "БазовыеПраваЭД,"
					+ "ВыполнениеОбменаБизнесСеть,"
					+ "ВыполнениеОбменаЭД,"
					+ "ДобавлениеИзменениеАдресныхСведений,"
					+ "ДобавлениеИзменениеАктовОРасхожденияхПриПриемкеТоваров,"
					...
					+ "ДобавлениеИзменениеПоложениеОПремировании,"
					+ "ДобавлениеИзменениеПравилаРасчетаПремий,"
					+ "ДобавлениеИзменениеРасчетПремийПоЛичнымПродажам,"
					+ "ПросмотрОтчетаДосьеКонтрагента,"
					+ "ИспользованиеУниверсальногоОтчета,"
					+ "НастройкаПараметровЭД,"
					+ "НастройкаСинхронизацииФайлов,"
					+ "ОтправкаSMS,"
					+ "ПолныеПрава,"
					+ "ЧтениеАктовОРасхожденияхПриПриемкеТоваров,"
					+ "ЧтениеПоложениеОПремировании,"
					+ "ЧтениеПравилаРасчетаПремий,"
					+ "ЧтениеРасчетПремийПоЛичнымПродажам,"
					+ "ЧтениеЭД";
					
	// Установка видимости страницы регистрационных карт.
	Элементы.СтраницаРегистрационныеКарты.Видимость = Пользователи.РолиДоступны(СписокРолей);
	
	// Установка видимости страницы подарочных сертификатов.
	СписокРолей = СписокРолей + ",ДобавлениеИзменениеНоменклатуры";
	Элементы.СтраницаПодарочныеСертификаты.Видимость = Пользователи.РолиДоступны(СписокРолей);
	
	// Установка видимости страницы дисконтных сертификатов.
	СписокРолей = СтрЗаменить(СписокРолей, ",ДобавлениеИзменениеНоменклатуры", ",ДобавлениеИзменениеИнформационныхКарт");
	Элементы.СтраницаДисконтныеКарты.Видимость = Пользователи.РолиДоступны(СписокРолей);
...
Процедура ДобавитьТовары(Таблица, ИдентификаторСтроки = Неопределено)
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.ТаблицаТоваров Цикл
		...
		тч.Сортировать("Дата Убыв");
		Если тч.Количество()>1 Тогда
			НоваяСтрока.Штрихкод=тч[0].штрихкод;
		КонецЕсли;
		//Если НоваяСтрока.Номенклатура.ВидНоменклатуры=справочники.ххх_Справочник.ВидНоменклатурыАлкоголь.Значение тогда
			НоваяСтрока.страна=?(ЗначениеЗАполнено(НоваяСтрока.Номенклатура),НоваяСтрока.Номенклатура.СтранаПроисхождения,Справочники.СтраныМира.ПустаяСсылка());
		//Иначе
			//НоваяСтрока.страна=?(ЗначениеЗАполнено(НоваяСтрока.Номенклатура),НоваяСтрока.Номенклатура.Производитель,Справочники.СтраныМира.ПустаяСсылка()); 
		//КонецЕсли;
		//Zorius	
		//
...
Процедура МагазинПриИзмененииСервер()
	...
	СписокРеквизитовМагазина = Новый Структура;
	СписокРеквизитовМагазина.Вставить("ВидМинимальныхЦенПродажи", "ВидМинимальныхЦенПродажи");
	СписокРеквизитовМагазина.Вставить("ПравилоЦенообразованияВидЦен", "ПравилоЦенообразования.ВидЦен");
	СтруктураМагазина = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Магазин, СписокРеквизитовМагазина);
	Объект.ВидЦены = СтруктураМагазина.ПравилоЦенообразованияВидЦен;
	Объект.ВидМинимальныхЦенПродажи = СтруктураМагазина.ВидМинимальныхЦенПродажи;
	Объект.УчитыватьЦеныМинимальные = ЗначениеЗаполнено(СтруктураМагазина.ВидМинимальныхЦенПродажи) И НЕ Объект.ЦеныПоВидуЦены;
	УстановитьДоступностьРеквизитаДействующихЦен();
	ИзменитьДоступностьЭлементовПриИзмененииЦеныПоВидуЦены();
КонецПроцедуры
...
Функция ТекстЗапросаПересчетаЦен()
	...
	Если ЦеныПоВидуЦены = 0 Тогда
		...
		|					ТОГДА 1
		|				КОГДА ЦеныНоменклатуры.Упаковка.Коэффициент = 0
		|					ТОГДА 1
		|				ИНАЧЕ ЦеныНоменклатуры.Упаковка.Коэффициент
		|			КОНЕЦ КАК Цена,
		...
		|		Подзапрос.Упаковка) КАК Подзапрос2
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Товары.НомерСтроки,
...
&НаКлиенте
Процедура УстановитьСтрану(Команда)
	Для каждого стр из Объект.Товары Цикл
		стр.Страна=Страна;
	КонецЦикла;
КонецПроцедуры
