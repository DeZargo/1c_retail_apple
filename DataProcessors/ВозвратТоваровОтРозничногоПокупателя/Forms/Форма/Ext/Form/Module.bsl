...
Процедура ОбработатьДанныеПоКодуСервер(СтруктураРезультат) Экспорт
	...
	Если СтрокаРезультата.Свойство("Карта") Тогда
		Если СтрокаРезультата.ЭтоРегистрационнаяКарта Тогда
			ПодключаемоеОборудованиеРТВызовСервера.ВставитьПредупреждениеОНевозможностиОбработкиКарт(СтруктураРезультат,
				СтрокаРезультата);
		Иначе
...
Функция ОбработатьДанныеИзТСДСервер(СтруктураПараметров) Экспорт
	...
	Результат = ПодключаемоеОборудованиеРТВызовСервера.ОбработатьДанныеПоНоменклатуреИзТСДСервер(ЭтотОбъект,
		СтруктураПараметров);
...
Функция ДобавитьНайденныеПозицииТоваров(СтруктураПараметров) Экспорт 
	...
	Иначе
		...
		ТекущаяСтрока = ПодключаемоеОборудованиеРТВызовСервера.ИнициализацияСтрокиТоваров(ЭтотОбъект,
			СтруктураПараметров, ДобавленаСтрока);
		...
		ИдентификаторСтроки = ПодключаемоеОборудованиеРТВызовСервера.ЗавершениеОбработкиСтрокиТоваров(ЭтотОбъект,
...
Процедура ОбработатьВыборДокументаПродажи(Выбор, ТекущаяСтрока) Экспорт
	...
	Если НЕ Выбор = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйДокумент)  Тогда
			...
			РезультатОтбораСтрок =
				ОбработкаТабличнойЧастиТоварыВызовСервера.ПолучитьСтрокиДокументаПродажиНаКлиента(ВыбранныйДокумент,
					МетаданныеНаименование, ТекущаяСтрока.Номенклатура, ТекущаяСтрока.Характеристика, УникальныйИдентификатор);
			...
			Если ЗначениеЗаполнено(РезультатОтбораСтрок)  Тогда
				...
				ОткрытьФорму("Обработка.ВозвратТоваровОтРозничногоПокупателя.Форма.ФормаВыбораСтрокиДокумента",
					СтруктураПараметров,,,,, Обработчик, Режим);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОповещениеОткрытьФормуАвторизацииЭТ(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатОткрытияФормы) = Тип("Структура") Тогда
		...
		Если НЕ ПараметрДействия.ПараметрыЭТ.ИспользоватьБезПодключенияОборудования Тогда
			...
			Оповещение = Новый ОписаниеОповещения("ОповещениеОткрытьФормуАвторизацииЭТЗавершение",
				ЭтотОбъект, ПараметрДействия);
		...
		Иначе
			...
			ОбработчикОповещения = Новый ОписаниеОповещения("ОповещениеВопросУспешностиОплатыКартой",
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий =
		Новый ФиксированнаяСтруктура(Документы.ВозвратТоваровОтПокупателя.ПараметрыУказанияСерий(Объект));
...
Процедура ПриОткрытии(Отказ)
	...
	ЗначениеСправочникиВидыОплатЧекаККМОплатаПодарочнымСертификатом =
		ПредопределенноеЗначение("Справочник.ВидыОплатЧекаККМ.ОплатаПодарочнымСертификатом");
	...
	МенеджерОборудованияКлиент.НачатьПодключениеОборудованиеПриОткрытииФормы(Неопределено,
		ЭтотОбъект, "СканерШтрихкода, СчитывательМагнитныхКарт");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.Оплата.Количество() <> 0 Тогда
		Сдача = Объект.Оплата.Итог("Сумма") - (Объект.Товары.Итог("Сумма") +
			?(Объект.ЦенаВключаетНДС, 0, Объект.Товары.Итог("СуммаНДС")));
	...
	Для Каждого СтрокаОплаты Из Объект.Оплата Цикл
		Если СтрокаОплаты.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ПлатежнаяКарта
			И НЕ ЗначениеЗаполнено(СтрокаОплаты.ЭквайринговыйТерминал) Тогда
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Объект.Касса = ЗначениеНастроекВызовСервера.ПолучитьКассуОрганизацииПоУмолчанию(Объект.Организация,
		,,Объект.Магазин);
...
&НаКлиенте
Процедура ЧекДатаПриИзменении(Элемент)
...
Процедура ТекстДокументаЕГАИСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИнтеграцияЕГАИСКлиент.ВыполнитьКомандуГиперссылки(ЭтотОбъект["Добавлен" + ИндексЭлемента],
		НавигационнаяСсылкаФорматированнойСтроки, ЭтотОбъект);
КонецПроцедуры
...
Процедура ТоварыДокументПродажиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоказатьВводЗначения(ОписаниеОповещения, ДатаС, НСтр("ru = 'Введите дату начала поиска документов:'"),
		ОбщегоНазначенияРТКлиентСервер.ПолучитьОписаниеТиповДаты());
КонецПроцедуры
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если ОбработкаТабличнойЧастиТоварыКлиент.НеобходимоОбновитьСтатусыСерий(Элементы.Товары,
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ОбработкаТабличнойЧастиТоварыКлиент.ОбновитьКэшированныеЗначенияДляУчетаСерий(Элемент,
		КэшированныеЗначения, ПараметрыУказанияСерий);
КонецПроцедуры
...
Процедура ТоварыПослеУдаления(Элемент)
	...
	Если ОбработкаТабличнойЧастиТоварыКлиент.НеобходимоОбновитьСтатусыСерий(Элемент,
			КэшированныеЗначения, ПараметрыУказанияСерий, Истина) Тогда
...
Процедура ТоварыУпаковкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработкаТабличнойЧастиТоварыКлиент.ВыбратьУпаковкуНоменклатуры(ДанныеВыбора,
		СтандартнаяОбработка, Элементы.Товары.ТекущиеДанные);
КонецПроцедуры
...
Процедура ТоварыХарактеристикаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработкаТабличнойЧастиТоварыКлиент.ВыбратьХарактеристикуНоменклатуры(ЭтотОбъект,
		Элемент, СтандартнаяОбработка, Элементы.Товары.ТекущиеДанные);
КонецПроцедуры
...
Процедура ТоварыЧекККМНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоказатьВводЗначения(ОписаниеОповещения, ДатаС, НСтр("ru = 'Введите дату чека:'"),
		ОбщегоНазначенияРТКлиентСервер.ПолучитьОписаниеТиповДаты());
КонецПроцедуры
...
Процедура ОплатаВидОплатыПриИзменении(Элемент)
	...
	ТекущаяСтрока.ПроцентКомиссии =
		ЭквайрингВызовСервера.ПолучитьПроцентКомиссииПоТерминалу(ТекущаяСтрока.ЭквайринговыйТерминал,
			ТекущаяСтрока.ВидОплаты, Истина);
...
Процедура ПолучитьВес(Команда)
	...
	ПодключаемоеОборудованиеРТКлиент.ПолучениеВесаСЭлектронныхВесовДляТабличнойЧасти(ОповещенияПриПолученииВеса,
		ЭтотОбъект);
КонецПроцедуры
...
Процедура ПечатьЗаявленияНаВозврат(Команда)
	...
	ПараметрыПечати.Вставить("ОтКого" , НСтр("ru = 'от'") +
		ОбщегоНазначенияРТКлиентСерверПовтИсп.ПолучитьСклонениеФИО(Объект.ФИОФизЛицаИП, 2));
	ПараметрыПечати.Вставить("ФИОПокупателя" ,
		ОбщегоНазначенияРТКлиентСерверПовтИсп.ПолучитьСклонениеФИО(Объект.ФИОФизЛицаИП, 5));
	...
	Если Не Руководитель = Неопределено Тогда
		Должность =
			ОбщегоНазначенияРТКлиентСерверПовтИсп.ПолучитьСклонениеФИО(РуководительДолжность, 3) + " " + Наименование;
...
Процедура ПечатьЗаявленияОбОшибочноПробитомЧеке(Команда)
	...
	ПараметрыПечати.Вставить("ОтКого" , НСтр("ru = 'от'") +
		ОбщегоНазначенияРТКлиентСерверПовтИсп.ПолучитьСклонениеФИО(Объект.ФИОФизЛицаИП, 2));
	ПараметрыПечати.Вставить("ФИОПродавца" ,
		ОбщегоНазначенияРТКлиентСерверПовтИсп.ПолучитьСклонениеФИО(Объект.ФИОФизЛицаИП, 5));
...
Процедура НайтиПоЧекуККМ(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Магазин) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "Объект.Магазин");
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ЧекНомер) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "Объект.ЧекНомер");
		ЕстьОшибки = Истина;
	КонецЕсли;
...
Процедура ПлатежнаяКарта(Команда)
	...
	Иначе
		...
		Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
		ОткрытьФорму("Справочник.ВидыОплатЧекаККМ.ФормаВыбора",
			Новый Структура("СтруктураПараметрыОтбора", Отбор),,,,, ОписаниеОповещения, Режим);
...
Процедура Подключаемый_ПробитиеЧекаПоДокументу(Команда)
	...
	Замер = ОценкаПроизводительностиРТКлиент.НачатьЗамер(Ложь,
		"Обработка.ВозвратТоваровОтРозничногоПокупателя.Форма.Форма.Команда.ПробитиеДокумента", Ложь);
	...
	ИначеЕсли ТипЗнчДокумент = Тип("ДокументСсылка.ОплатаОтПокупателяПлатежнойКартой") Тогда
		...
		Иначе
			...
			Если НЕ ПроверкаВозможностиРаботыСКартой(Документ,
					ПараметрыФормы, ПараметрДействия, ПараметрыКассыККМ, ПараметрыЭТ, ИмяКоманды) Тогда
				Возврат;
			КонецЕсли;
			...
			ОбработчикОповещения = Новый ОписаниеОповещения("ОповещениеОткрытьФормуАвторизацииЭТ", ЭтотОбъект,
				ДополнительныеПараметры);
			Режим = РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс;
			ОткрытьФорму("Справочник.ПодключаемоеОборудование.Форма.ФормаАвторизацииЭТ", ПараметрыФормы,,,,,
				ОбработчикОповещения, Режим);
...
Процедура ЗаполнитьСтавкиНДС()
	...
	УчетНДС.ЗаполнитьСписокВыбораСтавокНДС(Элементы, Объект.ЧекДата);
КонецПроцедуры
...
Процедура ЗаполнитьТоварыПоЧеку(Чек, ОчищатьТовары)
	...
	Если ОчищатьТовары Тогда
		...
		Для Каждого СтрокаТоваров Из Объект.Товары Цикл
			ОбработкаТабличнойЧастиТоварыСервер.ОбработатьСтрокуТЧСервер(СтрокаТоваров,
...
Процедура УказатьСерии(ТекущееПоле)
	...
	ОбработкаТабличнойЧастиТоварыКлиент.ПроверитьВозможностьУказанияСерий(ЭтотОбъект,
		ПараметрыУказанияСерий, ВводСерийРазрешен);
	...
	ОбработчикУказанияСерий = Новый ОписаниеОповещения("ОбработатьУказаниеСерий",
		ЭтотОбъект, ПараметрыФормыУказанияСерий);
	РежимБлокировки = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	
	ОткрытьФорму(ПараметрыФормыУказанияСерий.ИмяФормы, ПараметрыФормыУказанияСерий, ЭтотОбъект,,,,
...
Процедура ИнициализироватьОбработку(Параметры)
	...
	Объект.Склад = ЗначениеНастроекПовтИсп.ПолучитьСкладПоступленияПоУмолчанию(Объект.Магазин,
		,Объект.Склад, Объект.Ответственный);
	Объект.Касса = ЗначениеНастроекПовтИсп.ПолучитьКассуОрганизацииПоУмолчанию(Объект.Организация,
		,Объект.Касса, Объект.Магазин, Объект.Ответственный);
	...
	Объект.СтатьяДвиженияДенежныхСредств =
		ЗначениеНастроекПовтИсп.ПолучитьСтатьюДвиженияДенежныхСредств(Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту);
	Объект.АналитикаХозяйственнойОперации =
		ЗначениеНастроекПовтИсп.ПолучитьАналитикуХозяйственнойОперацииПоУмолчанию(Объект.АналитикаХозяйственнойОперации, Перечисления.ХозяйственныеОперации.ВозвратОтПокупателя);
	ЗначениеСправочникиВидыОплатЧекаККМОплатаПодарочнымСертификатом =
		Справочники.ВидыОплатЧекаККМ.ОплатаПодарочнымСертификатом;
...
Функция ЗаполнениеФизЛицаСервер()
	...
	Если Не ТипЗнч(Объект.ФизЛицо) = Тип("Строка") И ЗначениеЗаполнено(Объект.ФизЛицо) Тогда
		...
		УдостоверениеЛичности =
			РегистрыСведений.ДокументыФизическихЛиц.ДокументУдостоверяющийЛичностьФизлица(Объект.Физлицо);
...
Процедура НачалоВыбораВидаОплаты(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.ВидыОплатЧекаККМ.Форма.ФормаВыбора",
		Новый Структура("СтруктураПараметрыОтбора", Отбор), Элемент);
КонецПроцедуры
...
Процедура ПриИзмененииМагазина()
	...
	Объект.Склад = ЗначениеНастроекПовтИсп.ПолучитьСкладПоступленияПоУмолчанию(Объект.Магазин,
		,Объект.Склад, Пользователи.ТекущийПользователь());
	Объект.ЦенаВключаетНДС =
		ОбщегоНазначенияРТ.ПолучитьЗначениеРеквизитаВПривилегированномРежиме(Объект.Магазин.ПравилоЦенообразования,
			"ЦенаВключаетНДС");
...
Процедура ПриИзмененииРеквизитаОдиночнойОплатыСервер(СтруктураПараметров)
	...
	СтруктураПараметров.Вставить("ПроцентКомиссии",
		ЭквайрингВызовСервера.ПолучитьПроцентКомиссииПоТерминалу(СтруктураПараметров.ЭквайринговыйТерминал,
			СтруктураПараметров.ВидОплаты, Истина));
КонецПроцедуры
...
Функция ПараметрыФормыУказанияСерий(ТекущиеДанныеИдентификатор)
	...
	Возврат ОбработкаТабличнойЧастиТоварыСервер.ПоместитьСерииВХранилище(Объект,
...
Процедура НастроитьФормуПоДополнительнымПравам()
	...
	РазрешитьОформлениеВозвратовОтПокупателяБезДокументовПродажи =
		УправлениеПользователями.ПолучитьБулевоЗначениеПраваПользователя(
			ПланыВидовХарактеристик.ПраваПользователей.РазрешитьОформлениеВозвратовОтПокупателяБезДокументовПродажи, Ложь);
	
	Элементы.ТоварыДокументПродажи.АвтоОтметкаНезаполненного =
		НЕ РазрешитьОформлениеВозвратовОтПокупателяБезДокументовПродажи;
КонецПроцедуры
...
Функция СтруктураДанныхОбОрганизации(ДокументВозврата)
	...
	Если Выборка.Следующий() Тогда
		...
		Руководители = ФормированиеПечатныхФормСервер.ОтветственныеЛицаОрганизаций(Выборка.Организация,
			ОбщегоНазначения.ТекущаяДатаПользователя(),, Истина);
		...
		СтруктураРезультата.Вставить("Наименование" ,
			?(СокрЛП(Выборка.Организация.НаименованиеПолное) = "",
				Выборка.Организация.Наименование, Выборка.Организация.НаименованиеПолное));
...
Процедура ПроверитьПраваДоступаНаОбработку(Отказ)
	...
	ПравоНаЭквайринг = Пользователи.РолиДоступны("ДобавлениеИзменениеОплатОтПокупателяПлатежнойКартой")
		ИЛИ НЕ ПолучитьФункциональнуюОпцию("ИспользоватьОплатуПлатежнымиКартами");
...
Процедура ОтобразитьСформированныеДокументы(МассивСформированныхДокументов)
	...
	ОписаниеТипов = Новый ОписаниеТипов("ДокументСсылка.ВозвратТоваровОтПокупателя, ДокументСсылка.РасходныйКассовыйОрдер,
		|ДокументСсылка.ОплатаОтПокупателяПлатежнойКартой");
	...
	Для каждого Документ Из МассивСформированныхДокументов Цикл
		...
		ИначеЕсли ТипЗнчДокумент = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			Если ИнтеграцияЕГАИСВызовСервера.ИспользуетсяРегистрацияРозничныхПродажВЕГАИС(Объект.Организация,
					Объект.Магазин, Объект.Дата) Тогда
...
Процедура УстановитьДоступностьСоздатьДокументы();
	...
	Если НЕ МодифицированыСохраняемыеДанные
		ИЛИ Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаСформированныеДокументы Тогда
...
Процедура УстановитьКомандыВидовОплаты()
	...
	Если ПлатежнаяКарта = Неопределено Тогда
		Элементы.ОплатаПлатежнаяКарта.Заголовок = Элементы.ОплатаПлатежнаяКарта.Заголовок + НСтр("ru = '...'");
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьВидимостьПодсказкиТерминала()
	...
	Если ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыОплатЧекаККМ.ПлатежнаяКарта") Тогда
		...
		Иначе
			Элементы.ДекорацияПодсказка2.Заголовок = НСтр("ru = '<выберите кассу>'");
		КонецЕсли;
	Иначе
		Элементы.ДекорацияПодсказка2.Заголовок = "";
	КонецЕсли;
			...
			Для Каждого СтрокаОплаты Из Объект.Оплата Цикл
				Если СтрокаОплаты.ТипОплаты = ТипОплатыПлатежнаяКарта Тогда
					Если НЕ ЗначениеЗаполнено(СтрокаОплаты.ЭквайринговыйТерминал) Тогда
						...
						СтрокаОплаты.ПроцентКомиссии =
							ЭквайрингВызовСервера.ПолучитьПроцентКомиссииПоТерминалу(СтрокаОплаты.ЭквайринговыйТерминал,
								СтрокаОплаты.ВидОплаты, Истина);
...
Функция ПолучитьЭквайринговыйТерминалПоУмолчанию(ВидОплаты)
	...
	Если ЗначениеЗаполнено(Объект.Касса) Тогда
		Результат = Справочники.ЭквайринговыеТерминалы.ЭквайринговыйТерминалПоУмолчанию(Объект.Касса,
			Объект.Организация, Объект.Магазин, ВидОплаты);
	КонецЕсли;
...
Процедура ПолучитьНеобходимыеДанныеДляРаботыСКартой(Документ, ПараметрыФормы, ПараметрДействия, ПараметрыКассыККМ, ПараметрыЭТ)
	...
	ПараметрыЭТ.ИспользоватьБезПодключенияОборудования = ПараметрыЭТ.ИспользоватьБезПодключенияОборудования
...
Процедура ЗавершитьОплатуКартой(ПараметрДействия)
	...
	ПробитЧек = Ложь;
	ПараметрыДокумента = ОбщегоНазначенияРТВызовСервера.ПолучитьЗначенияРеквизитовОбъекта(ПараметрДействия.Документ,
	...
	Если РезультатЗаписи = Истина Тогда
		...
		Иначе
			...
			ПодключаемоеОборудованиеРТКлиент.НачатьПробитиеЧекаКОВыполнить(Оповещение, Объект,
				ПараметрыДляЧека, УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ВходящиеДанныеДляФРСервер(ДокументНаККМ, РаспределениеВыручкиПоСекциям, НомерЧека)
	...
	Возврат Документы.РасходныйКассовыйОрдер.ПодготовитьДанныеДляПробитияЧека(ДокументНаККМ,
		РаспределениеВыручкиПоСекциям, НомерЧека);
КонецФункции 
...
Процедура НапечататьЧекКлиентЗавершение(РезультатВыполнения, ПараметрыОперации) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ПараметрыДокумента =
			ОбщегоНазначенияРТВызовСервера.ПолучитьЗначенияРеквизитовОбъекта(ПараметрыОперации.ДокументНаККМ,
				Новый Структура("КассаККМ, НомерЧекаККМ"));
