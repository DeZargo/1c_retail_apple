...
Процедура ПроверитьНаличиеКомиссионнойМаркируемойПродукции(ДокументОбъект, Отказ) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаТовары.Номенклатура.ТабачнаяПродукция, ЛОЖЬ) КАК ТабачнаяПродукция
	|ИЗ
	|	ТаблицаТовары КАК ТаблицаТовары
	|ГДЕ
	|	(ЕСТЬNULL(ТаблицаТовары.Номенклатура.АлкогольнаяПродукция, ЛОЖЬ)
	|			ИЛИ ЕСТЬNULL(ТаблицаТовары.Номенклатура.ТабачнаяПродукция, ЛОЖЬ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.ТабачнаяПродукция Тогда 
			ТекстСообщения = НСтр("ru = 'В строке %1 содержится табачная продукция. Прием на комиссию табачной продукции запрещен.'");
