...
#Область Справочники_НоменклатураПоставщиков_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.12.04";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "РегистрыСведений.НоменклатураПоставщиков.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("3218ade9-fddd-4476-8717-79c7cfecc2ed");
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.НоменклатураПоставщиков,"
	...
	Обработчик.ПроцедураПроверки    = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.БлокируемыеОбъекты   = "РегистрСведений.НоменклатураПоставщиков,"
		+ "РегистрСведений.НоменклатураКонтрагентовБЭД";
...
#КонецОбласти
	...
	Обработчик.Версия = "2.2.12.05";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыРТ.ПересчитатьСуммуНаличныхВОРП";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("4fe55461-b678-4b0f-b337-46096fee0c62");
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ЧитаемыеОбъекты   = "Документ.ОтчетОРозничныхПродажах,РегистрНакопления.ДенежныеСредстваККМ,Документ.ВыемкаДенежныхСредствИзКассыККМ";
	Обработчик.ИзменяемыеОбъекты = "Документ.ОтчетОРозничныхПродажах,РегистрНакопления.ДенежныеСредстваККМ,Документ.ВыемкаДенежныхСредствИзКассыККМ";
	Обработчик.Комментарий = НСтр("ru = 'Пересчет суммы наличных в отчетах о розниых продажах.'");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ОбновлениеИнформационнойБазыРТ.БезЗарегистрированияДанныхКОбработкеДляПерехода";
	Обработчик.ПроцедураПроверки    = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.БлокируемыеОбъекты   = "Документ.ОтчетОРозничныхПродажах,РегистрНакопления.ДенежныеСредстваККМ,Документ.ВыемкаДенежныхСредствИзКассыККМ";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.12.09";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыРТ.ИзменитьТоварыНаСкладахВРамкахРезерва";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("25758374-3c22-480e-9c64-05eb69f4d8ff");
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ЧитаемыеОбъекты   = "Документ.ОтчетОРозничныхПродажах, РегистрНакопления.ТоварыНаСкладах";
	Обработчик.ИзменяемыеОбъекты = "РегистрНакопления.ТоварыНаСкладах";
	Обработчик.Комментарий = НСтр("ru = 'Пересчет зарезервированного ранее товара, возвращенного после закрытия смены'");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ОбновлениеИнформационнойБазыРТ.БезЗарегистрированияДанныхКОбработкеДляПерехода";
	Обработчик.ПроцедураПроверки    = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ПересчитатьСуммуНаличныхВОРП(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		|	ДенежныеСредстваККМ.ДоговорПлатежногоАгента КАК ДоговорПлатежногоАгента,
		|	ВЫБОР
		|		КОГДА ДенежныеСредстваККМ.ДоговорПлатежногоАгента = &ПустойДоговор
		...
		ЗапросДвижения.Параметры.Вставить("ПустойДоговор", Справочники.ДоговорыПлатежныхАгентов.ПустаяСсылка());
		...
		|	ВыемкаДенежныхСредствИзКассыККМ.ДоговорПлатежногоАгента КАК ДоговорПлатежногоАгента
		|ИЗ
		...
		|	И ВыемкаДенежныхСредствИзКассыККМ.ДоговорПлатежногоАгента = &ПустойДоговор";
		
		ЗапросВыемка.УстановитьПараметр("ОтчетОРозничныхПродажах", Выборка.ОтчетОРозничныхПродажах);
		ЗапросВыемка.Параметры.Вставить("ПустойДоговор", Справочники.ДоговорыПлатежныхАгентов.ПустаяСсылка());
		...
		ЗапросВыемкаДвижения.Параметры.Вставить("ПустойДоговор", Справочники.ДоговорыПлатежныхАгентов.ПустаяСсылка());
