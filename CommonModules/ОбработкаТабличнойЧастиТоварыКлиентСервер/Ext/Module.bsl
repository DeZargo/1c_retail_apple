...
// Функция возвращает структуру заполнения ставки НДС.
//
Функция СтруктураПараметровСтавкиНДСВТЧ(Объект) Экспорт
	
	СтруктураПараметровСтавкиНДС = Новый Структура;
	СтруктураПараметровСтавкиНДС.Вставить("Дата", 			Объект.Дата);
	СтруктураПараметровСтавкиНДС.Вставить("Магазин", 		Объект.Магазин);
	СтруктураПараметровСтавкиНДС.Вставить("ВидОперации", 	Объект.ВидОперации);
...
// Функция возвращает структуру заполнения ставки НДС. Если в объекте нет реквизита СистемаНалогообложения,
// то устанавливается Неопределено(для Заказа покупателя). Далее структура используется в 
// ОбработкаТабличнойЧастиТоварыСервер.ЗапонитьСтавкуНДСВСтрокеПоПараметрам
//
Функция СтруктураПараметровСтавкиНДССкладВидНалогаВШапке(Объект) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "СистемаНалогообложения") Тогда
		...
		СтруктураПараметровСтавкиНДС.Вставить("ВидНалогаВШапке", Объект.СистемаНалогообложения);
		
	Иначе
		
		СтруктураПараметровСтавкиНДС.Вставить("ВидНалогаВШапке", Неопределено);
...
// Функция возвращает структуру заполнения Вида налога.
//
Функция СтруктураПараметровЗаполненияВидаНалога(Объект) Экспорт
		
	СтруктураПараметровВидаНалога = Новый Структура;
	СтруктураПараметровВидаНалога.Вставить("Дата", 			Объект.Дата);
	СтруктураПараметровВидаНалога.Вставить("Магазин", 		Объект.Магазин);
	СтруктураПараметровВидаНалога.Вставить("Организация", 	Объект.Организация);
	СтруктураПараметровВидаНалога.Вставить("ВидОперации", 	Объект.ВидОперации);
	СтруктураПараметровВидаНалога.Вставить("ЧекККМПродажа",	Объект.ЧекККМПродажа);
		
	Возврат СтруктураПараметровВидаНалога;
...
// Рассчитывает сумму НДС от суммы через количество
Функция РассчитатьСуммуНДСПоСтрочно(Сумма, Количество, ПроцентНДС, ЦенаВключаетНДС = Истина) Экспорт
	...
	Для Каждого Итер из Коэффициенты Цикл
	...
	Для Каждого РаспределеннаяСумма Из РаспределенныеСуммы Цикл
		СуммаНДС = СуммаНДС + Окр(РассчитатьСуммуНДС(РаспределеннаяСумма, ПроцентНДС, ЦенаВключаетНДС), 2);
	КонецЦикла;
...
Функция ПолучитьЗначениеФлагаНДСПоСтрочно(СтрокаТЧ) Экспорт
	...
	Если ТипЗнч(СтрокаТЧ) = Тип("Структура") ИЛИ ТипЗНЧ(СтрокаТЧ) = Тип("ДанныеФормыЭлементКоллекции") Тогда 
		Если СтрокаТЧ.Свойство("НДСПоСтрочно", НДСПоСтрочно) И СтрокаТЧ.Свойство("Количество") Тогда
			Если НДСПоСтрочно = Неопределено Тогда
				НДСПоСтрочно = Ложь;
			КонецЕсли;
		Иначе
			НДСПоСтрочно = Ложь;
