	...
	Иначе
		Если Результат.Источник = "ПодключаемоеОборудование" Тогда
			Если Результат.ИмяСобытия = "ScanData"
				...
				ТекстОшибки = "";
				ОбработатьМаркуСредствамиГОСИС = Ложь;
				ОбработатьМаркуСредствамиГОСИС(Форма, ДанныеШтрихкода, ТекстОшибки, ОбработатьМаркуСредствамиГОСИС);
				
				Если ЗначениеЗаполнено(ТекстОшибки) Тогда 
					ОбработатьОшибочноеСканирование(Форма, ТекстОшибки, ТекКод);
				Иначе
					Если ОбработатьМаркуСредствамиГОСИС Тогда 
			...
			Если Результат.ИмяСобытия = "ScanData" Тогда
				...
				Иначе
					Если ОбработатьМаркуСредствамиГОСИС Тогда 
						...
						//На случай, если считали марку, а цена не установлена.
...
Процедура ОбработатьМаркуСредствамиГОСИС(Форма, ДанныеШтрихкода, ТекстОшибки, ОбработатьМаркуСредствамиГОСИС)
	
	НовыеДанныеШтрихкода    = ШтрихкодированиеИСКлиентСервер.ОбработатьДанныеШтрихкода(ДанныеШтрихкода);
	МаркируемаяПродукция    = ПодключаемоеОборудованиеРТВызовСервера.МаркируемаяПродукция(НовыеДанныеШтрихкода.Штрихкод);
	ИмяФормы                = Форма.ИмяФормы;
	
	Если МаркируемаяПродукция.ЭтоАлкоголь И (ИмяФормы = "Обработка.РМКУправляемыйРежим.Форма.Форма" 
		Или ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокумента" 
		Или ИмяФормы = "Документ.РеализацияТоваров.Форма.ФормаДокумента") Тогда 
		
		ОбработатьМаркуСредствамиГОСИС = Истина;
		
	ИначеЕсли МаркируемаяПродукция.ЭтоТабак И ИмяФормы = "Документ.РеализацияТоваров.Форма.ФормаДокумента" Тогда 
		
		ОбработатьМаркуСредствамиГОСИС = Истина;
		ТекстОшибки = НСтр("ru = 'Реализация табачной продукции оптом не поддерживается!'");
		
	ИначеЕсли МаркируемаяПродукция.ЭтоТабак И ИмяФормы = "Документ.ПоступлениеТоваров.Форма.ФормаДокумента" Тогда 
		
		ОбработатьМаркуСредствамиГОСИС = Истина;
		ТекстОшибки = НСтр("ru = 'Сканирование табачной продукции следует выполнять в форме проверки и подбора табака!'");
		
	ИначеЕсли МаркируемаяПродукция.ЭтоТабак И (ИмяФормы = "Обработка.РМКУправляемыйРежим.Форма.Форма" 
		Или ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокумента" Или ИмяФормы = "Документ.ВозвратТоваровОтПокупателя.Форма.ФормаДокумента" 
...
Процедура ОбработатьОшибочноеСканирование(Форма, ТекстОшибки, Штрихкод)
	
	СтруктураДанныхШтрихкода = Новый Структура;
	СтруктураДанныхШтрихкода.Вставить("ТекстОшибки",               ТекстОшибки);
	СтруктураДанныхШтрихкода.Вставить("Штрихкод",                  Штрихкод);
	СтруктураДанныхШтрихкода.Вставить("ПредставлениеНоменклатуры", "");
	СтруктураДанныхШтрихкода.Вставить("Номенклатура",              "");
	
	ПараметрыОткрытияФормы = Новый Структура();
	ПараметрыОткрытияФормы.Вставить("ДанныеШтрихкода", СтруктураДанныхШтрихкода);
