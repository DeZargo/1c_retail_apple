...
Функция РассчитатьСуммуНДС(СтрокаТЧ, ЦенаВключаетНДС = Истина)
	...
	ФлагНДСПострочно = ОбработкаТабличнойЧастиТоварыКлиентСервер.ПолучитьЗначениеФлагаНДСПоСтрочно(СтрокаТЧ);
	
	Если ФлагНДСПострочно Тогда
		СуммаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДСПоСтрочно(СтрокаТЧ.Сумма, СтрокаТЧ.Количество, ПроцентНДС, ЦенаВключаетНДС);
	Иначе
		СуммаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДС(СтрокаТЧ.Сумма, ПроцентНДС, ЦенаВключаетНДС);
	КонецЕсли;
