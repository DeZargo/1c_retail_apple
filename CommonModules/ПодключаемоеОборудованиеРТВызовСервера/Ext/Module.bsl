...
Функция ДобавитьВложенныеМарки(Форма, СтруктураПараметров, МассивМонотоварных = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.ШтрихкодУпаковкиЕГАИС)
			...
			СтруктураВложенная = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПараметров);
...
Функция МаркируемаяПродукция(Штрихкод) Экспорт
	...
	СтруктураМаркируемойПродукции.Вставить("ЭтоТабак",    ШтрихкодированиеМОТП.ЭтоШтрихкодТабачнойПродукции(Штрихкод,,Истина));
