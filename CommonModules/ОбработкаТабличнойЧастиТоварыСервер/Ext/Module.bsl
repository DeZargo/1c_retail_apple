...
Процедура ЗаполнитьСтавкуНДСВСтрокеСкладВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДССкладВСтроке", СтруктураПараметровДействия) Тогда
		ЗапонитьСтавкуНДСВСтрокеПоПараметрам(ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения);
	КонецЕсли;
...
Процедура ПересчитатьСуммуНДСВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДС", СтруктураПараметровДействия) Тогда
		...
		НДСПоСтрочно = ОбработкаТабличнойЧастиТоварыКлиентСервер.ПолучитьЗначениеФлагаНДСПоСтрочно(ТекущаяСтрока);
		
		Если НДСПоСтрочно Тогда
			ТекущаяСтрока.СуммаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДСПоСтрочно(ТекущаяСтрока.Сумма,
																	ТекущаяСтрока.Количество,
																	ТекПроцентНДС,
																	СтруктураПараметровДействия.ЦенаВключаетНДС);
		Иначе
			ТекущаяСтрока.СуммаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДС(ТекущаяСтрока.Сумма,
															ТекПроцентНДС,
															СтруктураПараметровДействия.ЦенаВключаетНДС);
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтатусовМаркировкиГосИС(ТабличнаяЧасть, Отказ) Экспорт 
	...
	Для каждого Строка Из ТабличнаяЧасть Цикл
		Если Строка.СтатусПроверкиГосИС = 2 Тогда 
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла; 
	
	Если Отказ Тогда 
		ТекстСообщения = НСтр("ru = 'Не заполнены марки табачной продукции.'");
...
Процедура ЗаполнитьСкладПродажиВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСкладПродажи", СтруктураПараметровДействия) Тогда
		...
		Если ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Услуга Тогда
			...
			Иначе
				Если ЗначениеЗаполнено(СтруктураПараметровДействия.Магазин) Тогда
...
Процедура ЗаполнитьОрганизациюПродажиВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьОрганизациюПродажи", СтруктураПараметровДействия) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.Организация) И ЗначениеЗаполнено(СтруктураПараметровДействия.Магазин) Тогда
...
Процедура ЗаполнитьНеобходимостьВводаАкцизнойМарки(ТаблицаФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	УчитыватьТабак = Истина;
	
	Если НЕ ДополнительныеПараметры = Неопределено Тогда
		Если ДополнительныеПараметры.Свойство("Дата") Тогда
			ПроверяемаяДата = ДополнительныеПараметры.Дата;
		КонецЕсли;
		Если ДополнительныеПараметры.Свойство("УчитыватьТабак") Тогда 
			УчитыватьТабак = ДополнительныеПараметры.УчитыватьТабак;
		КонецЕсли;
	КонецЕсли;
	...
	|	ВЫБОР
	|	КОГДА ТаблицаЗапроса.Номенклатура.ТабачнаяПродукция
	|		ТОГДА "+?(УчитыватьТабак = Истина, "ИСТИНА", "ЛОЖЬ")+"
	|	КОГДА ТаблицаЗапроса.Номенклатура.ВидАлкогольнойПродукцииЕГАИС.Маркируемый И НЕ ТаблицаЗапроса.Номенклатура.ВидНоменклатуры.ПродаетсяВРозлив
	|		ТОГДА ИСТИНА
	|	ИНАЧЕ 
	|		ЛОЖЬ
	|	КОНЕЦ
...
Процедура ЗаполнитьНеобходимостьВводаКодаМаркировки(ТаблицаФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ТаблицаЗапроса.Номенклатура.ОбувнаяПродукция, ЛОЖЬ)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
...
Процедура ПроверитьНеобходимостьВводаКодаМаркировки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаКодаМаркировки") Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда 
			...
			ЭтоОбувнаяПродукция = ТекущаяСтрока.Номенклатура.ОбувнаяПродукция;
			
			Если НЕ ОперацияСДенежнымиСредствами И ВестиУчетМаркировкиОбувнойПродукции 
				И ЭтоОбувнаяПродукция Тогда
				
				ТекущаяСтрока.НеобходимостьВводаКодаМаркировки = Истина;
			Иначе 
...
Процедура ПроверитьНеобходимостьВводаАкцизнойМарки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаАкцизнойМарки", СтруктураПараметровДействия) Тогда
		...
		ВестиУчетТабачнойПродукцииМОТП = Ложь;
		Если ТипЗнч(СтруктураПараметровДействия) = Тип("Структура") 
			И СтруктураПараметровДействия.Свойство("ВестиУчетТабачнойПродукцииМОТП") Тогда
			
			ВестиУчетТабачнойПродукцииМОТП = СтруктураПараметровДействия.ВестиУчетТабачнойПродукцииМОТП;
		...
		Если Не ОперацияСДенежнымиСредствами Тогда 
			...
			Если ИспользуетсяРегистрацияРозничныхПродажВЕГАИС И ДанныеПродукции.ЭтоАлкогольнаяПродукция Тогда 
			...
			ИначеЕсли ВестиУчетТабачнойПродукцииМОТП И ДанныеПродукции.ЭтоТабачнаяПродукция Тогда 
...
//      * Организация 	- организация, в которой определяется вид налога. Используется если в ТаблицаДанных нет колонки "Организация".
//      * ВидОперации 	- вид операции документа, Перечисление.ВидыОперацийЧекККМ
//      * ЧекККМПродажа - документ продажи, заполнеяется при возврате Документ.ЧекККМ.
//
Процедура ЗаполнитьВидНалогаВТЧСервер(ТаблицаДанных, СтруктураПараметров) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	ТаблицаЗапроса КАК ТаблицаЗапроса
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПрименениеСистемНалогообложения.СрезПоследних(&Дата, ) КАК ПрименениеСистемНалогообложенияСрезПоследних
	...
	Иначе
		...
		Запрос.УстановитьПараметр("ТаблицаТоваров", ТаблицаДанных.Выгрузить());
...
// Процедура заполнения идентификатора номенклатуры поставщика в строках (ЭДО, Бизнес-сеть).
//
...
Процедура ЗаполнитьФлагНДСПоСтрочноСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ЗаполнитьФлагНДСПоСтрочно") Тогда
		...
		ТекущаяСтрока.НДСПоСтрочно = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Номенклатура, "ТабачнаяПродукция");
		
	КонецЕсли;

КонецПроцедуры

// Процедура заполнения признака маркируемая продукция ГосИС.
//
Процедура ЗаполнитьМаркируемаяПродукцияГосИС(ТаблицаФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	|	ТаблицаЗапроса.НомерСтроки,
	|	ТаблицаЗапроса.Номенклатура,
	|	ЕСТЬNULL(ТаблицаЗапроса.Номенклатура.ТабачнаяПродукция, ЛОЖЬ) КАК НеобходимостьВводаМаркируемойПродукцияГосИС
	|ИЗ
	|	ТаблицаЗапроса КАК ТаблицаЗапроса";
	...
	Для Каждого СтрТабл Из ТаблицаФормы Цикл
		...
		СтрТабл.МаркируемаяПродукцияГосИС = СтрокаРезультата.НеобходимостьВводаМаркируемойПродукцияГосИС;
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаМаркируемойПродукцииГосИС") Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда 
			...
			Если НЕ ОперацияСДенежнымиСредствами И ДанныеПродукции.ЭтоТабачнаяПродукция Тогда
				ТекущаяСтрока.МаркируемаяПродукцияГосИС = Истина;
			Иначе 
...
Процедура ЗаполнитьНДСПоСтрочно(ТаблицаФормы) Экспорт
	...
	|	ТаблицаЗапроса.НомерСтроки,
	|	ТаблицаЗапроса.Номенклатура,
	|	ТаблицаЗапроса.Номенклатура.ТабачнаяПродукция КАК ТабачнаяПродукция
	|ИЗ
	|	ТаблицаЗапроса КАК ТаблицаЗапроса";
	...
	Для Каждого СтрТабл Из ТаблицаФормы Цикл
		...
		Если СтрокаРезультата.ТабачнаяПродукция Тогда
...
//											   строки таблицы
//
Процедура ЗапонитьСтавкуНДСВСтрокеПоПараметрам(ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения)
