	...
	Если ПроверятьМарки Тогда
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			Если СтрокаТовара.Маркируемый
				...
				Если СтрокаТовара.ТабачнаяПродукция Тогда 
					ТипМаркиСтрокой = НСтр("ru = 'код маркировки'");
				ИначеЕсли СтрокаТовара.АлкогольнаяПродукция Тогда 
					ТипМаркиСтрокой = НСтр("ru = 'акцизная марка'");
				КонецЕсли;
				
				ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + ТипМаркиСтрокой;
				...
				Если НЕ ОсобыйВыводСообщения Тогда
					Если ЭтоПервоеСообщение Тогда
						ОбщегоНазначения.СообщитьПользователю(
							НСтр("ru = 'Не хватает данных для передачи:'"),
							ДокументОбъект);
...
Функция ДанныеАлкогольнойПродукции(Номенклатура) Экспорт
	...
	СтруктураРезультат.Вставить("ЭтоТабачнаяПродукция"        , Ложь);
...
Функция ЕстьАлкогольнаяПродукцияЕГАИС(ТаблицаТовары) Экспорт
	...
	Для Каждого Стр Из ТаблицаТовары Цикл
		Если Стр.НеобходимостьВводаАкцизнойМарки И Стр.Номенклатура.АлкогольнаяПродукция Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ТребуетсяОформлениеДокументовЕГАИС(Основание, МетаданныеДокументаЕГАИС) Экспорт
	...
	ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не реализована процедура ""ИнтеграцияЕГАИСРТ.ТребуетсяОформлениеДокументовЕГАИС""'"));
...
Функция СформироватьДанныеПоАкцизнымМаркам(Организация, Магазин, Штрихкод, КлючСвязи, Номенклатура, Характеристика) Экспорт 
	...
	Иначе
		АкцизнаяМарка = ШтрихкодыУпаковок[0].Ссылка;
		СтруктураЗаполнения.Вставить("АкцизнаяМарка", 		АкцизнаяМарка);
		СтруктураЗаполнения.Вставить("ШтрихкодУпаковки", 	АкцизнаяМарка);
		СтруктураЗаполнения.Вставить("Справка2", 			Справка2АкцизнойМарки(АкцизнаяМарка, ОрганизацияЕГАИС));
	КонецЕсли;
