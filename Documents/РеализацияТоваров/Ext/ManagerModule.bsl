...
Функция ПечатьМ15(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Если Номер = 1 Тогда // первая строка
...
Функция ОбработатьДанныеШтрихкодаЕдиницыАлкогольнойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	СтруктураПоиска.Вставить("Упаковка", 		?(ДанныеШтрихкода.Упаковка = Неопределено, Справочники.УпаковкиНоменклатуры.ПустаяСсылка(), ДанныеШтрихкода.Упаковка));
	
	СтрокиТовара = Объект.Товары.НайтиСтроки(СтруктураПоиска);
	
	Если СтрокиТовара.Количество() = 0 Тогда
		СтрокаТовара = Новый Структура("Номенклатура, Характеристика, Упаковка, Количество, Штрихкод",,,,0,);
	...
	СтрокаТовара.Характеристика       = ДанныеШтрихкода.Характеристика;
	СтрокаТовара.Упаковка             = ДанныеШтрихкода.Упаковка;
