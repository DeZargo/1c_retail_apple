...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Объект.РасшифровкаПлатежа.Количество() > 0 И НЕ ЗначениеЗаполнено(Объект.СистемаНалогообложения) Тогда
		
			РасшифровкаПлатежаДокументРасчетовСКонтрагентомПриИзмененииСервер(
				Объект.РасшифровкаПлатежа[0].ПолучитьИдентификатор());
...
Процедура РасшифровкаПлатежаПередУдалением(Элемент, Отказ)
	...
	ПерезаполнитьСистемуНалогообложения = Объект.ПробиватьЧекиПоКассеККМ 
...
Процедура ОбработатьРеквизитыФормы()
	...
	Элементы.ГруппаСНО.Видимость 								= ПробиватьЧекиПоКассеККМ;
	Элементы.КассаККМ.Видимость 								= ПробиватьЧекиПоКассеККМ;
	Элементы.РасшифровкаПлатежаСистемаНалогообложения.Видимость = ПробиватьЧекиПоКассеККМ;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ОбщаяКомандаФискальныеОперации", "Видимость", Объект.ПробитЧек И ПробиватьЧекиПоКассеККМ);
...
Процедура РасшифровкаПлатежаДокументРасчетовСКонтрагентомПриИзмененииСервер(ИдентификаторСтроки)
	...
	Если ЗначениеЗаполнено(ДокументРасчетовСКонтрагентом) Тогда
		...
		Если СНОДокумента <> Неопределено Тогда
			...
			СтрокиРасшифровки = Объект.РасшифровкаПлатежа.НайтиСтроки(
				Новый Структура("ДокументРасчетовСКонтрагентом", ДокументРасчетовСКонтрагентом));
			
			Для каждого СтрокаРасшифровки Из СтрокиРасшифровки Цикл
				СтрокаРасшифровки.СистемаНалогообложения = СНОДокумента.Получить(ДокументРасчетовСКонтрагентом);
			КонецЦикла;
