...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьЗаполнениеМарокТабака(Отказ);
...
Процедура ПроверитьЗаполнениеМарокТабака(Отказ)
	...
	Если ВестиУчетТабачнойПродукцииМОТП Тогда 
		...
		Если Не ИнтеграцияМОТП.ДанныеДокументаСоответствуютДаннымУпаковок(Ссылка) Тогда 
			Отказ = Истина;
			ТекстСообщения = НСтр("ru = 'Неверно заполнены данные по табачным маркам.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,"Объект",Отказ);
			Возврат;
		КонецЕсли;
