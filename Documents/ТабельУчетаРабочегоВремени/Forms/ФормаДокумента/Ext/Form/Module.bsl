...
Функция ТабельДанныеОВремениИзПредставления(Знач ПредставлениеДанныхОВремени, ДанныеТекущейСтроки, НомерДня, Отказ = Ложь)
	...
	Если ДанныеОВремени = Неопределено Или ДанныеОВремени.БуквенноеОбозначение = "" И ДанныеОВремени.Часы = 0  Тогда 
		Возврат Новый Структура("ВидВремени, БуквенноеОбозначение, Часы", ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыИспользованияРабочегоВремени.ПустаяСсылка"), "", 0);
	ИначеЕсли ДанныеОВремени.БуквенноеОбозначение = "" Тогда 
		Если ПустаяСтрока(ДанныеТекущейСтроки["ВидВремениПоУмолчаниюОбозначение" + НомерДня]) Тогда 
			Возврат Новый Структура("ВидВремени, БуквенноеОбозначение, Часы",  ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисления.ВидыИспользованияРабочегоВремени.Явка"), "Я", ДанныеОВремени.Часы);
		...
		Иначе
			Возврат Новый Структура("ВидВремени, БуквенноеОбозначение, Часы",   ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыИспользованияРабочегоВремени.ПустаяСсылка"), "", ДанныеОВремени.Часы);	
		КонецЕсли;	
	Иначе	
		ОписаниеВидаВремени = ОписаниеВидовВремени.Получить(ДанныеОВремени.БуквенноеОбозначение);
