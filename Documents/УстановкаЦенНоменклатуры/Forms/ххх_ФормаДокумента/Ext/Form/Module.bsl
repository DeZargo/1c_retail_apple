...
Процедура ОповещениеПоискаПоШтрихкоду(Структура, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Структура) Тогда
		...
		Иначе			
			Элементы.ДеревоЦен.ТекущаяСтрока=ПоискПоВесовомуШтрихКоду(штрихкод);
			ДеревоЦенНоменклатураПриИзменении("");
		КонецЕсли;
		//Zorius
		ПоискПоШтрихкоду("");
		//Zorius
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//Zorius
	Если не ПараметрыЗаписи.Свойство("Отказ") и ПараметрыЗаписи.РежимЗаписи=РежимЗаписиДокумента.Проведение Тогда 
		Предупреждение="";
		ПолучитьПредупреждениеЕслиПроцентНаценкиМеньшеНуля(Предупреждение);
		Если ЗначениеЗаполнено(Предупреждение) тогда
			Отказ=Истина;
			ПараметрыЗаписи.Вставить("Отказ",Отказ);
			Предупреждение="Провести документ?"+Символы.ВК+Предупреждение;
			ПоказатьВопрос(Новый ОписаниеОповещения("ОбработкаВопросаОПроведенииДокумента",ЭтаФОрма,ПараметрыЗаписи),Предупреждение,РежимДиалогаВопрос.ДаНет);		
		КонецЕсли;
	ИначеЕсли ПараметрыЗаписи.Свойство("Отказ") Тогда
...
КонецПроцедуры
//Zorius
Функция ПолучитьПредупреждениеЕслиПроцентНаценкиМеньшеНуля(Предупреждение)
	ПоместитьЦеныВТабличнуюЧасть(Объект, Ложь);
	Запрос=новый запрос;
	Запрос.УстановитьПараметр("тз",Объект.Товары.выгрузить());
	Запрос.УстановитьПараметр("Опт",справочники.ххх_Справочник.ЗакупочнаяЦена.Значение);
	Запрос.УстановитьПараметр("Розн",справочники.ххх_Справочник.РозничнаяЦена.Значение);
	Запрос.УстановитьПараметр("ДокументОснование",Объект.ДокументОснование);
	ВидыЦен=Объект.ВидыЦен.Выгрузить();
	Если ВидыЦен.Количество()=2 Тогда
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |////////////////////////////////////////////////////////////////////////////////
		             |ВЫБРАТЬ
		             |	опт.Номенклатура КАК Номенклатура,
		             |	(розн.ЦенаРозница / опт.ЦенаОпт - 1) * 100 КАК Наценка,
		             |	ВЫБОР
		             |		КОГДА опт.НомерСтроки / 2 - (ВЫРАЗИТЬ(опт.НомерСтроки / 2 - 0.5 КАК ЧИСЛО(15, 0))) = 0
		             |			ТОГДА опт.НомерСтроки / 2
		             |		ИНАЧЕ ВЫРАЗИТЬ(опт.НомерСтроки / 2 - 0.5 КАК ЧИСЛО(15, 0))
		             |	КОНЕЦ КАК НомерСтроки
		             |ИЗ
		             |	опт КАК опт
		             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ розн КАК розн
		             |		ПО опт.Номенклатура = розн.Номенклатура
		             |ГДЕ
		             |	розн.ЦенаРозница / опт.ЦенаОпт - 1 <= 0";
	ИначеЕсли ВидыЦен.Количество()=1 и ЗначениеЗАполнено(ВидыЦен.Найти(справочники.ххх_Справочник.РозничнаяЦена.Значение)) Тогда
		Запрос.Текст="ВЫБРАТЬ
		             |	тз.Номенклатура КАК Номенклатура,
		             |	тз.Цена КАК Цена,
		             |	тз.ВидЦены КАК ВидЦены,
		             ...
		             |ВЫБРАТЬ
		             |	тз.НомерСтроки КАК НомерСтроки,
		             |	(тз.Цена / ЦеныНоменклатурыСрезПоследних.Цена - 1) * 100 КАК Наценка,
		             |	тз.Номенклатура КАК Номенклатура
		             |ПОМЕСТИТЬ нац
		             |ИЗ
		             |	тз КАК тз
		             |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &Опт) КАК ЦеныНоменклатурыСрезПоследних
		             |		ПО тз.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
		             |;
		             |
		             |////////////////////////////////////////////////////////////////////////////////
		             |ВЫБРАТЬ
		             |	нац.НомерСтроки КАК НомерСтроки,
		             |	нац.Наценка КАК Наценка,
		             |	нац.Номенклатура КАК Номенклатура
		             |ИЗ
		             |	нац КАК нац
		             |ГДЕ
		             |	нац.Наценка <= 0
		             ...
		             |УПОРЯДОЧИТЬ ПО
		             |	НомерСтроки";	
	//ИначеЕсли ЗначениеЗаполнено(Объект.ДокументОснование) и ЗначениеЗАполнено(ВидыЦен.Найти(справочники.ххх_Справочник.РозничнаяЦена.Значение)) Тогда
	//	Запрос.Текст="ВЫБРАТЬ
	//	             |	тз.Номенклатура КАК Номенклатура,
	//	             |	тз.Цена КАК Цена,
	//	             |	тз.ВидЦены КАК ВидЦены,
	...
	//	             |ВЫБРАТЬ
	//	             |	тз.НомерСтроки КАК НомерСтроки,
	//	             |	(ПоступлениеТоваровТовары.Цена / тз.Цена - 1) * 100 КАК Наценка
	//	             |ПОМЕСТИТЬ нац
	//	             |ИЗ
	//	             |	тз КАК тз,
	...
	//	             |ГДЕ
	//	             |	нац.Наценка <= 0";
	//ИначеЕсли ВидыЦен.Количество()=1 и ЗначениеЗАполнено(ВидыЦен.Найти(справочники.ххх_Справочник.ЗакупочнаяЦена.Значение))Тогда
	//	Запрос.Текст="ВЫБРАТЬ
	//	             |	тз.Номенклатура КАК Номенклатура,
	//	             |	тз.Цена КАК Цена,
	//	             |	тз.ВидЦены КАК ВидЦены,
	...
	Если не Запрос.Текст="" Тогда			 
		...
		Пока выборка.Следующий() Цикл
			Предупреждение=Предупреждение+"В строке: "+выборка.НомерСтроки+" по номенклатуре: "+выборка.номенклатура+" процент наценки = "+выборка.Наценка+"%."+Символы.ВК;
		КонецЦикла;
	КонецЕсли;
...
//Zorius
&НаКлиенте
Процедура ОбработкаВопросаОПроведенииДокумента(ВарОтв,ПараметрыЗаписи) экспорт
	Если ВарОтв=КодВозвратаДиалога.Нет Тогда
		ПараметрыЗаписи.отказ=Истина;
		ЭтаФорма.Записать(ПараметрыЗаписи);
	Иначе
		ПараметрыЗаписи.отказ=Ложь;
		ЭтаФорма.Записать(ПараметрыЗаписи);
	КонецЕсли;	
КонецПроцедуры
//Zorius
...
Процедура ПриОткрытии(Отказ)
	...
	//
	//Если ВыбранныеЦены.Количество()=2 или выбранныеЦены.Количество()=1 и ВыбранныеЦены[0].ссылка=ПолучитьРозничнуюЦену() Тогда
	//	Элементы.ДеревоЦенПроцентНаценки.видимость=Истина;
	//Иначе
	//	Элементы.ДеревоЦенПроцентНаценки.видимость=Ложь;
	//КонецЕсли;
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда  
		ЗапретитьМенятьЦенуНаТехГдеУстановленыОфисныеНаценкиЦены();
	КонецЕсли;
	//Zorius
КонецПроцедуры
...
Процедура ДеревоЦенПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	//Элементы.ДеревоЦен.Обновить();	
	//ТекущиеДанные = Элементы.ДеревоЦен.ДанныеСтроки(Элементы.ДеревоЦен.ТекущаяСтрока);//Элементы.ДеревоЦен.ТекущиеДанные;
	...
	//			Если Элементы.ДеревоЦен.ТекущаяСтрока <> ЭлементНоменклатура.ПолучитьИдентификатор() Тогда
	...
	//	КонецЦикла;
	//	
	//	ДеревоЦенНоменклатураПриИзмененииСервер(Элементы.ДеревоЦен.ТекущаяСтрока, НЕ ИзмененаЦена);
	//	
	//КонецЕсли;
	//
...
Процедура ДеревоЦенПриАктивизацииЯчейки(Элемент)
	...
	//Zorius
	Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.ФиксЦенаФиксПроц) тогда
		Элемент.ТекущийЭлемент.ТолькоПросмотр=истина;
	Иначе
		Элемент.ТекущийЭлемент.ТолькоПросмотр=Ложь;
	КонецЕсли;
	//Если ЗначениеЗаполнено(НАйти(Элемент.ТекущийЭлемент.Имя,"ДеревоЦенПроцентНаценки")) и Элемент.ТекущиеДанные.НельзяИзменять тогда
	//	Элемент.ТекущийЭлемент.ТолькоПросмотр=истина;
	//Иначе
	//	Элемент.ТекущийЭлемент.ТолькоПросмотр=Ложь;
	//КонецЕсли;
...
Процедура ДеревоЦенНоменклатураПриИзмененииСервер(ИдентификаторСтроки, РассчитатьЦены = Истина)
	...
	//Zorius		
	//Древо=ДанныеФормыВЗначение(ЭтаФорма.ДеревоЦен,Тип("ДеревоЗначений"));
	//номка=Древо.строки.выгрузитьКолонку("Номенклатура");
	//Тч=Новый таблицаЗначений;
	//Тч.Колонки.Добавить("Номка",Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	//КвалификаторыЧисла = Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Любой);
	//Тч.колонки.Добавить("Колво",Новый ОписаниеТипов("Число",КвалификаторыЧисла));
	//Для каждого эл из номка цикл
	//	стр=Тч.Добавить();
	//	стр.номка=эл;
	//	стр.колво=1;
	//КонецЦикла;
	//Тч.Свернуть("Номка","Колво");
	//Повторы=Тч.Найти(2,"Колво");
	//Если ЗначениеЗАполнено(ПОвторы) Тогда		
	//	Сообщить("Номенклатура: "+повторы.номка+"уже есть в документе.");
	//	ТекущаяСтрока.Номенклатура=Справочники.Номенклатура.ПустаяСсылка();
	//КонецЕСли;
	//
	//Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда	
	//	Запрос=Новый запрос;
	//	Запрос.УстановитьПараметр("номка",ТекущаяСтрока.Номенклатура);
	//	Запрос.УстановитьПараметр("дата",Объект.Дата);
	//	Запрос.Текст="ВЫБРАТЬ
	//	             |	ххх_НаценкиСрезПоследних.Номенклатура КАК Номенклатура,
	//	             |	ххх_НаценкиСрезПоследних.Наценка_ФиксЦена КАК Наценка_ФиксЦена
	//	             |ИЗ
	//	             |	РегистрСведений.ххх_Наценки.СрезПоследних(&дата, Номенклатура = &номка) КАК ххх_НаценкиСрезПоследних
	//	             |ГДЕ
	//	             |	ххх_НаценкиСрезПоследних.Наценка_ФиксЦена <> 0";
	//	выборка=запрос.Выполнить().Выбрать();
	//	
	//	Пока выборка.Следующий() цикл
	//		стр=Древо.строки.Найти(выборка.номенклатура);
	//		стр.НельзяИзменять=Истина;
	//	КонецЦикла;
	//	ЗначениеВДанныеФормы(Древо,ЭтаФорма.ДеревоЦен);
	//КонецЕсли;
	//Zorius
...
&НаКлиенте
Процедура ДеревоЦенПроцентНаценкиПриИзмененииКочетов(Элемент)
	
	ТекущаяСтрока = Элементы.ДеревоЦен.ТекущиеДанные;
	//ВидыЦенВыбранные=ВыбранныеЦены.Выгрузить();
	//НеобходимостьЗапрашиватьЗакупочнуюЦену
	Если ТекущаяСтрока <> Неопределено Тогда
		//Если ВыбранныеЦены.Количество()=2 и ВыбранныеЦены[1].выбрана=истина и ВыбранныеЦены[0].выбрана=истина Тогда
		//	ТекущаяСтрока["ВидЦены"+ПолучитьИдентификиаторРозничнойЦены()]=
		//		ТекущаяСтрока["ВидЦены"+ПолучитьИдентификиаторЗАкупочнойЦены()]*
		//		ТекущаяСтрока.процентНаценки/100+ТекущаяСтрока["ВидЦены"+ПолучитьИдентификиаторЗАкупочнойЦены()];
		//ИначеЕсли ВыбранныеЦены.Количество()=2 и ВыбранныеЦены.НайтиПоИдентификатору(ПолучитьРозничнуюЦену()) Тогда
		Если ТекущаяСтрока.Свойство("ВидЦены"+ПолучитьИдентификиаторЗАкупочнойЦены()) и ТекущаяСтрока.Свойство("ВидЦены"+ПолучитьИдентификиаторРозничнойЦены()) Тогда
		    ЗакупочнаяЦена=ТекущаяСтрока["ВидЦены"+ПолучитьИдентификиаторЗАкупочнойЦены()];
		Иначе
			ЗакупочнаяЦена=ПолучитьЗакупочнуюЦенуНаДату(Объект.Дата,ТекущаяСтрока.Номенклатура);
		КонецЕсли;
		Если ЗакупочнаяЦена<>0 Тогда
			ТекущаяСтрока["ВидЦены"+ПолучитьИдентификиаторРозничнойЦены()]=ЗакупочнаяЦена*ТекущаяСтрока.процентНаценки/100+ЗакупочнаяЦена;
		Иначе
			Сообщить("Не установленна закупочная цена.");
		КонецЕсли;
		//Иначе
		//	Элементы.процентНаценки.виджимость=Ложь;
		//КонецЕсли;
	КонецЕсли;
...
функция ПолучитьЗакупочнуюЦенуНаДату(дата,Номенклатура)
	
	Отбор=Новый структура;
	Отбор.Вставить("Номенклатура",Номенклатура);
	Отбор.Вставить("ВидЦены",Справочники.ххх_Справочник.ЗакупочнаяЦена.Значение);
	Цены=РегистрыСведений.ЦеныНоменклатуры.СрезПоследних(Дата,Отбор);
...
функция ПолучитьИдентификиаторЗАкупочнойЦены()
	возврат СтрЗаменить(Справочники.ххх_Справочник.ЗакупочнаяЦена.значение.УникальныйИдентификатор(),"-","");
КонецФункции

функция ПолучитьИдентификиаторРозничнойЦены()
	возврат СтрЗаменить(Справочники.ххх_Справочник.РозничнаяЦена.значение.УникальныйИдентификатор(),"-","");
КонецФункции
	...
	Если ТаблицаНоменклатуры.Количество() > 0 Тогда
		...
		Если СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
			...
			Если Ценообразование.ПроверитьСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, ТекстОшибкиПриПроверке) Тогда
				...
				//Zorius
				//ДобавитьВыбранноеПолеСКД(ГруппировкаДетальныеЗаписи, "НельзяИзменять");
				//Zorius
				Если ИспользоватьХарактеристикиНоменклатуры Тогда
				...
				ВнешниеНаборыДанных.Вставить(
					"ТаблицаНоменклатуры",
					ТаблицаНоменклатуры.Скопировать(, "Номенклатура, Характеристика"));
...
Процедура ВычислитьЦеныПоДаннымИБ(ТаблицаНоменклатуры, МассивСтрокВидовЦен = Неопределено, ТолькоНезаполненные = Ложь)
	...
	Древо=ДанныеФормыВЗначение(ЭтаФорма.ДеревоЦен,Тип("ДеревоЗначений"));
	Запрос=Новый запрос;
	Запрос.УстановитьПараметр("номка",Древо.строки.выгрузитьКолонку("Номенклатура"));
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |ИЗ
	             |	РегистрСведений.ххх_Наценки.СрезПоследних(&дата, Номенклатура В (&номка)) КАК ххх_НаценкиСрезПоследних
	             |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	             |				&дата,
	             |				Номенклатура В (&номка)
	             |					И ВидЦены = &ВидЦены) КАК ЦеныНоменклатурыСрезПоследних
	             |		ПО ххх_НаценкиСрезПоследних.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	             |ГДЕ
	             |	ххх_НаценкиСрезПоследних.Наценка_ФиксЦена <> 0
	             |	И ЦеныНоменклатурыСрезПоследних.Цена <> 0";
	выборка=запрос.Выполнить().Выбрать();
	Розница="ВидЦены" + СтрЗаменить(Справочники.ххх_Справочник.РозничнаяЦена.Значение.УникальныйИдентификатор(), "-", "");
	Пока выборка.Следующий() цикл
		стр=Древо.строки.Найти(выборка.номенклатура);
		Если выборка.Наценка_ФиксЦена<0 Тогда
			стр.ФиксЦенаФиксПроц=1;
		Иначе
			стр.ФиксЦенаФиксПроц=2;
		КонецЕсли;
		стр.процентНаценки=100*(стр[Розница]/выборка.Цена-1);
		стр.ОфиснаяНаценка=выборка.Наценка_ФиксЦена;
	КонецЦикла;
	//Для каждого стр из древо цикл
	//	стр.процентНаценки=;
	//КонецЦикла;
	ЗначениеВДанныеФормы(Древо,ЭтаФорма.ДеревоЦен);
	//Zorius
...
//
&НаСервереБезКонтекста
Процедура ДобавитьУпаковкиСтрокиТаблицыЦенВМассив(
			Знач ЭлементДерева,
			Знач ТаблицаНоменклатуры,
			Знач МассивВидовЦен,
			МассивУпаковок)
	
	СтруктураПоиска = Новый Структура("Номенклатура, Характеристика",
								ЭлементДерева.Номенклатура,
								ЭлементДерева.Характеристика);
...
Процедура ЗагрузитьЗначенияБазовыхЦен(
	...
	Если МассивСтрокВидовЦен.Количество() > 0 Тогда
		...
		Запрос = Новый Запрос("
			...
			|	ЦеныНоменклатурыСрезПоследних.Цена + ЦеныНоменклатурыСрезПоследних.Цена * &ПроцентИзмененияЦены / 100 КАК Цена
			|ИЗ
			|	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
			|			&ДатаДокумента,
			|			(Номенклатура, Характеристика) В
...
Процедура ЗагрузитьТабличнуюЧастьТовары()
	...
	|
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаТовары.Номенклатура.ВидНоменклатуры.ИспользованиеХарактеристик
	|			= ЗНАЧЕНИЕ(Перечисление.ВариантыВеденияДополнительныхДанныхПоНоменклатуре.ОбщиеДляВидаНоменклатуры) ТОГДА 
	|				ИСТИНА
	|		КОГДА ВременнаяТаблицаТовары.Номенклатура.ВидНоменклатуры.ИспользованиеХарактеристик
	|			= ЗНАЧЕНИЕ(Перечисление.ВариантыВеденияДополнительныхДанныхПоНоменклатуре.ИндивидуальныеДляНоменклатуры) ТОГДА
	|				ИСТИНА
	|	ИНАЧЕ ЛОЖЬ КОНЕЦ КАК ХарактеристикиИспользуются,
	|
	|	ВременнаяТаблицаТовары.ВидЦены КАК ВидЦены,
	...
	|		КОГДА ЦеныНоменклатуры.Цена <> 0
	|			ТОГДА 100 * (ВременнаяТаблицаТовары.Цена - ВЫБОР
	|					КОГДА ЕСТЬNULL(ЦеныНоменклатуры.Упаковка, &ПустаяУпаковка) = ВременнаяТаблицаТовары.Упаковка
	...
	|														Номенклатура,
	|														Характеристика
	|													ИЗ ВременнаяТаблицаТовары)) КАК ЦеныНоменклатуры
	|			ПО ЦеныНоменклатуры.Номенклатура   = ВременнаяТаблицаТовары.Номенклатура
	|			 И ЦеныНоменклатуры.Характеристика = ВременнаяТаблицаТовары.Характеристика
	|			 И ЦеныНоменклатуры.ВидЦены        = ВременнаяТаблицаТовары.ВидЦены
	|	
...
Процедура ПереформироватьТаблицуЦен(ПересчитатьЦены)
	...
	//Если НЕ Параметры.Ключ.Пустая()  тогда
		...
		Древо=ДанныеФормыВЗначение(ЭтаФорма.ДеревоЦен,Тип("ДеревоЗначений"));
		Розница="ВидЦены" + СтрЗаменить(Справочники.ххх_Справочник.РозничнаяЦена.Значение.УникальныйИдентификатор(), "-", "");
		Закуп="ВидЦены" + СтрЗаменить(Справочники.ххх_Справочник.ЗакупочнаяЦена.Значение.УникальныйИдентификатор(), "-", "");
		Запрос=Новый запрос;
		Запрос.УстановитьПараметр("номка",Древо.строки.выгрузитьКолонку("Номенклатура"));
		Запрос.УстановитьПараметр("дата",КонецДня(Объект.Дата));
		Запрос.УстановитьПараметр("ВидЦены",Справочники.ххх_Справочник.ЗакупочнаяЦена.Значение);
...
Процедура ПостроитьДеревоЦен()
	...
	Для Каждого ВидЦены Из ВыбранныеЦены Цикл
		...
		Если ВидЦены.Выбрана ИЛИ ВидЦены.Влияет Тогда
			...
			ИмяКолонки = ВидЦены.ИмяКолонки;
			//Zorius
			Если ВидЦены.Ссылка=Справочники.ххх_Справочник.РозничнаяЦена.Значение Тогда
				ДеревоЗначений.Колонки.Добавить(
					ИмяКолонки,
					Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 1, ДопустимыйЗнак.Неотрицательный)),
					ВидЦены.Наименование);
			ИначеЕсли ВидЦены.Ссылка=Справочники.ххх_Справочник.ЗакупочнаяЦена.Значение Тогда
				ДеревоЗначений.Колонки.Добавить(
					ИмяКолонки,
					Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)),
					ВидЦены.Наименование);
			ИначеЕсли ВидЦены.Ссылка=Справочники.ххх_Справочник.ВидЦеныШокиЗачеркнутая.Значение Тогда
				ДеревоЗначений.Колонки.Добавить(
					ИмяКолонки,
					Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)),
					ВидЦены.Наименование);
	...
	//Zorius
	ДеревоЗначений.Колонки.Добавить("ФиксЦенаФиксПроц",Новый ОписаниеТипов("Число"));
	ДеревоЗначений.Колонки.Добавить("ПроцентНаценки");
	//ДеревоЗначений.Колонки.Добавить("Код");
	ДеревоЗначений.Колонки.Добавить("ОфиснаяНаценка");
	//Zorius
...
Процедура ЗапретитьМенятьЦенуНаТехГдеУстановленыОфисныеНаценкиЦены()
	//Zorius
	ВидыЦен=Объект.ВидыЦен.Выгрузить(,"ВидЦены");
	Если (ЗначениеЗаполнено(ВидыЦен.Найти(Справочники.ххх_Справочник.РозничнаяЦена.Значение)) и ЗначениеЗаполнено(ВидыЦен.Найти(Справочники.ххх_Справочник.ЗакупочнаяЦена.Значение))) Тогда
		...
		выборка=запрос.Выполнить().Выбрать();
		Розница="ВидЦены" + СтрЗаменить(Справочники.ххх_Справочник.РозничнаяЦена.Значение.УникальныйИдентификатор(), "-", "");
		Опт="ВидЦены" + СтрЗаменить(Справочники.ххх_Справочник.ЗакупочнаяЦена.Значение.УникальныйИдентификатор(), "-", "");
		Пока выборка.Следующий() цикл
			стр=Древо.строки.Найти(выборка.номенклатура);
			Если выборка.Наценка_ФиксЦена<0 Тогда
				стр.ФиксЦенаФиксПроц=1;
	...
	ИначеЕсли ЗначениеЗаполнено(ВидыЦен.Найти(Справочники.ххх_Справочник.РозничнаяЦена.Значение)) Тогда
