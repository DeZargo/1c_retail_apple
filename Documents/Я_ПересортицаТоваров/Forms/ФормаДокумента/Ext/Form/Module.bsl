...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Замер = ОценкаПроизводительностиРТКлиент.НачатьЗамер(Ложь, 
	                                            "Документ.Я_ПересортицаТоваров.ФормаДокумента.Запись",
                                                           Ложь);
...
Процедура ТоварыНоменклатураСписаниеПриИзменении(Элемент)
	...
	//ValMa - первая проверка на алкоголь
	ОчиститьСообщения();
	ЭтоАлкоголь = ОпределитьАлкогольнуюПродукцию(ТекущаяСтрока.Номенклатура);
	Если ЭтоАлкоголь Тогда
		ТекущаяСтрока.Номенклатура = ПолучитьПустуюСсылкуНаТовар();
		Сообщить("Запрещено выбирать к списанию алкогольную продукцию!");
		Возврат;
	КонецЕсли;	
	//---
	...
	//ValMa
	Если ТекущаяСтрока.Номенклатура <> ПолучитьПустуюСсылкуНаТовар() Тогда
		ВидЦеныРозницы = ПолучитьРозничнуюЦену();
		РозничнаяЦена  = УстановитьЦенуТовара(ТекущаяСтрока.Номенклатура, ВидЦеныРозницы);
		
		ВидЦеныЗакупа  = ПолучитьЗакупочнуюЦену();
		ЗакупочнаяЦена = УстановитьЦенуТовара(ТекущаяСтрока.Номенклатура, ВидЦеныЗакупа);
		
		//вторая проверка по цене (цена розницы не должна быть меньше цены закупа)
		Если ЗакупочнаяЦена > РозничнаяЦена Тогда
			Сообщить("Закупочная цена ("+ЗакупочнаяЦена+") больше Розничной цены ("+РозничнаяЦена+") у товара: "+ТекущаяСтрока.Номенклатура+"!");
			//ТекущаяСтрока.Номенклатура = ПолучитьПустуюСсылкуНаТовар();
			//ТекущаяСтрока.НоменклатураОприходование = ПолучитьПустуюСсылкуНаТовар();
			Возврат;
		КонецЕсли;	
		
		//ТекущаяСтрока.Цена = УстановитьЦенуТовара(ТекущаяСтрока.Номенклатура);
		ТекущаяСтрока.Цена = РозничнаяЦена;
		РасчетСуммыСписания();
	Иначе
		ТекущаяСтрока.ххх_СуммаСписания = 0;
	КонецЕсли;
	//---
...
&НаСервере
Функция ОпределитьАлкогольнуюПродукцию(Ссылка)
	Возврат Ссылка.АлкогольнаяПродукция;	
КонецФункции
...
Процедура ТоварыНоменклатураОприходованиеПриИзменении(Элемент)
	...
	//ValMa - первая проверка на алкоголь
	ОчиститьСообщения();
	ЭтоАлкоголь = ОпределитьАлкогольнуюПродукцию(ТекущиеДанные.НоменклатураОприходование);
	Если ЭтоАлкоголь Тогда
		ТекущиеДанные.НоменклатураОприходование = ПолучитьПустуюСсылкуНаТовар();
		Сообщить("Запрещено выбирать к оприходованию алкогольную продукцию!");
		Возврат;
	КонецЕсли;
	//---	
	...
	//ValMa
	Если ТекущиеДанные.НоменклатураОприходование <> ПолучитьПустуюСсылкуНаТовар() Тогда 
		ВидЦеныРозницы = ПолучитьРозничнуюЦену();
		РозничнаяЦена  = УстановитьЦенуТовара(ТекущиеДанные.НоменклатураОприходование, ВидЦеныРозницы);
		
		ВидЦеныЗакупа  = ПолучитьЗакупочнуюЦену();
		ЗакупочнаяЦена = УстановитьЦенуТовара(ТекущиеДанные.НоменклатураОприходование, ВидЦеныЗакупа);
		
		//вторая проверка по цене (цена розницы не должна быть меньше цены закупа)
		Если ЗакупочнаяЦена > РозничнаяЦена Тогда
			Сообщить("Закупочная цена ("+ЗакупочнаяЦена+") больше Розничной цены ("+РозничнаяЦена+") у товара: "+ТекущиеДанные.НоменклатураОприходование+"!");
			//ТекущиеДанные.Номенклатура = ПолучитьПустуюСсылкуНаТовар();
			//ТекущиеДанные.НоменклатураОприходование = ПолучитьПустуюСсылкуНаТовар();
			Возврат;
		КонецЕсли;			
		//ТекущиеДанные.ЦенаОприходование = УстановитьЦенуТовара(ТекущиеДанные.НоменклатураОприходование);
		ТекущиеДанные.ЦенаОприходование = РозничнаяЦена;
		РасчетСуммыОприходования();
	Иначе
		ТекущиеДанные.ххх_СуммаОприходования = 0;
	КонецЕсли;
	//---
...
&НаКлиенте
Процедура ТоварыКоличествоСписаноПриИзменении(Элемент)
...
&НаКлиенте
Процедура РасчетСуммыСписания()
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекущаяСтрока.Цена) И ЗначениеЗаполнено(ТекущаяСтрока.ххх_КоличествоСписано) Тогда
		ТекущаяСтрока.ххх_СуммаСписания = ТекущаяСтрока.Цена * ТекущаяСтрока.ххх_КоличествоСписано;
	Иначе
		ТекущаяСтрока.ххх_СуммаСписания = 0;
	КонецЕсли;
	Если ЗначениеЗаполнено(ТекущаяСтрока.ххх_СуммаСписания) И ЗначениеЗаполнено(ТекущаяСтрока.ххх_СуммаОприходования) Тогда
		ТекущаяСтрока.ххх_ДельтаСписаниеОприходование = ТекущаяСтрока.ххх_СуммаОприходования - ТекущаяСтрока.ххх_СуммаСписания;	
		Если ТекущаяСтрока.ххх_ДельтаСписаниеОприходование < 0 Тогда
			ТекущаяСтрока.ххх_НедостачаПревышение = "Недостача";
		ИначеЕсли ТекущаяСтрока.ххх_ДельтаСписаниеОприходование > 0 Тогда
			ТекущаяСтрока.ххх_НедостачаПревышение = "Превышение";
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ТоварыКоличествоОприходованоПриИзменении(Элемент)
...
&НаКлиенте
Процедура ТоварыЦенаОприходованиеПриИзменении(Элемент)
...
&НаКлиенте
Процедура РасчетСуммыОприходования()
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекущаяСтрока.ЦенаОприходование) И ЗначениеЗаполнено(ТекущаяСтрока.ххх_КоличествоОприходовано) Тогда
		ТекущаяСтрока.ххх_СуммаОприходования = ТекущаяСтрока.ЦенаОприходование * ТекущаяСтрока.ххх_КоличествоОприходовано;
	Иначе
		ТекущаяСтрока.ххх_СуммаОприходования = 0;
	КонецЕсли;
	Если ЗначениеЗаполнено(ТекущаяСтрока.ххх_СуммаСписания) И ЗначениеЗаполнено(ТекущаяСтрока.ххх_СуммаОприходования) Тогда
		ТекущаяСтрока.ххх_ДельтаСписаниеОприходование = ТекущаяСтрока.ххх_СуммаОприходования - ТекущаяСтрока.ххх_СуммаСписания;	
		Если ТекущаяСтрока.ххх_ДельтаСписаниеОприходование < 0 Тогда
...
&НаСервере
Функция УстановитьЦенуТовара(СсылкаТовар, ВидЦены)
	
	ЦенаТовара = 0;

	//ВидЦены = ПолучитьРозничнуюЦену();
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Если РезультатЗапроса.Пустой() Тогда
		Сообщить("У товара: "+СсылкаТовар+", не назначена цена: "+ВидЦены+"!");
		Возврат ЦенаТовара;
	Иначе
...
&НаСервереБезКонтекста
Функция ПолучитьПустуюСсылкуНаТовар()
	Возврат Справочники.Номенклатура.ПустаяСсылка();
КонецФункции

&НаКлиенте
Процедура Товарыххх_СуммаСписанияПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекущаяСтрока.Цена) И 
		ТекущаяСтрока.ххх_СуммаСписания>0 Тогда
		ТекущаяСтрока.ххх_КоличествоСписано = ТекущаяСтрока.ххх_СуммаСписания/ТекущаяСтрока.Цена;
	КонецЕсли;
...
&НаКлиенте
Процедура Товарыххх_СуммаОприходыванияПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекущаяСтрока.ЦенаОприходование) И 
		ТекущаяСтрока.ххх_СуммаОприходования>0 Тогда
		ТекущаяСтрока.ххх_КоличествоОприходовано = ТекущаяСтрока.ххх_СуммаОприходования/ТекущаяСтрока.ЦенаОприходование;
	КонецЕсли;
