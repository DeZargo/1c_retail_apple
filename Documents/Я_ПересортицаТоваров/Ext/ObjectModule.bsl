...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.Я_ПересортицаТоваров.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//ValMa
	ЭтотОбъект.Сумма = ЭтотОбъект.Товары.Итог("ххх_ДельтаСписаниеОприходование");
	Если ЭтотОбъект.Сумма > 0 Тогда 
		ЭтотОбъект.ПревышениеНедостача = "Превышение";
	ИначеЕсли ЭтотОбъект.Сумма < 0 Тогда 
		ЭтотОбъект.ПревышениеНедостача = "Недостача";
	КонецЕсли;
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриказНаПроведениеИнвентаризацииТоваров") 
	 ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОрдерНаОтражениеРезультатовПересчетовТоваров") Тогда
		...
		Если ЗначениеЗаполнено(Магазин)
			...
			ТекстСообщения = НСтр("ru='В магазине ""%1%"" используется ордерная схема для отражения излишков и недостач, поэтому ""(Яблоко) пересортица товаров"" нужно вводить по основанию.'");
