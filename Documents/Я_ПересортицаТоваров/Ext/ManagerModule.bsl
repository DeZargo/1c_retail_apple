...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "Я_АктПересортицыТоваров";
	КомандаПечати.Представление = НСтр("ru = '(Яблоко) Акт о пересортице товаров'");
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства) Экспорт
	...
	|ИЗ
	|	Документ.Я_ПересортицаТоваров КАК ДанныеДокумента
	|ГДЕ
	...
	|	ТаблицаТовары.ххх_КоличествоСписано КАК Количество,
	|	ТаблицаТовары.Цена КАК Цена,
	|	&Склад КАК Склад,
	|	&АналитикаХозяйственнойОперации КАК АналитикаХозяйственнойОперации
	|ПОМЕСТИТЬ ВтТаблицаТоварыСписание
	|ИЗ
	|	Документ.Я_ПересортицаТоваров.Товары КАК ТаблицаТовары
	|ГДЕ
	...
	//|	ТаблицаТовары.Количество КАК Количество,
	|	ТаблицаТовары.ххх_КоличествоОприходовано КАК Количество,
	|	&Склад КАК Склад,
	|	&АналитикаХозяйственнойОперации КАК АналитикаХозяйственнойОперации
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Я_АктПересортицыТоваров") Тогда

		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Я_АктПересортицыТоваров",
				НСтр("ru = '(Яблоко) Акт о пересортице товаров'"),
...
Функция ПечатьАктаПересортицыТоваров(МассивОбъектов, ОбъектыПечати)
	...
	|ИЗ
	|	Документ.Я_ПересортицаТоваров КАК Документ
	|ГДЕ
	...
	//ValMa
	|	ТаблицаТовары.ххх_КоличествоСписано КАК КоличествоСписано,
	|	ТаблицаТовары.Цена КАК ЦенаСписание,
	|	ТаблицаТовары.ххх_СуммаСписания КАК СуммаСписание,
	//---
	...
	//ValMa
	|	ТаблицаТовары.ххх_КоличествоОприходовано КАК КоличествоОприходовано,
	|	ТаблицаТовары.ЦенаОприходование КАК ЦенаОприходование,
	|	ТаблицаТовары.ххх_СуммаОприходования КАК СуммаОприходование,
	//---
	...
	//|	ТаблицаТовары.Количество КАК Количество,
	//ValMa
	|	ТаблицаТовары.ххх_ДельтаСписаниеОприходование КАК РазницаСумм,
	//---
	|	ТаблицаТовары.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.Я_ПересортицаТоваров.Товары КАК ТаблицаТовары
	...
	СинонимДокумента = НСтр("ru='(Яблоко) Акт о пересортице товаров'");

	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПересортицаТоваров_АктОПересортицеТоваров";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Я_ПересортицаТоваров.Я_ПФ_MXL_АктОПересортицеТоваров");
	...
	//ValMa
	ОбластьКоличествоСписаниеШапка      = Макет.ПолучитьОбласть("ШапкаТаблицы|КолСписание");
	ОбластьЦенаСписаниеШапка            = Макет.ПолучитьОбласть("ШапкаТаблицы|ЦенаСписание");
	ОбластьСуммаСписаниеШапка           = Макет.ПолучитьОбласть("ШапкаТаблицы|СуммаСписание");
	ОбластьКоличествоОприходованиеШапка = Макет.ПолучитьОбласть("ШапкаТаблицы|КолОприходование");
	ОбластьЦенаОприходованиеШапка       = Макет.ПолучитьОбласть("ШапкаТаблицы|ЦенаОприходование");
	ОбластьСуммаОприходованиеШапка      = Макет.ПолучитьОбласть("ШапкаТаблицы|СуммаОприходование");
	//--
	...
	//ValMa
	ОбластьКоличествоСписаниеСтрока      = Макет.ПолучитьОбласть("Строка|КолСписание");
	ОбластьКоличествоОприходованиеСтрока = Макет.ПолучитьОбласть("Строка|КолОприходование");
	
	ОбластьЦенаСписаниеСтрока            = Макет.ПолучитьОбласть("Строка|ЦенаСписание");
	ОбластьСуммаСписаниеСтрока           = Макет.ПолучитьОбласть("Строка|СуммаСписание");

	ОбластьЦенаОприходованиеСтрока       = Макет.ПолучитьОбласть("Строка|ЦенаОприходование");
	ОбластьСуммаОприходованиеСтрока      = Макет.ПолучитьОбласть("Строка|СуммаОприходование");
	//--
	
	ОбластьДанныхСтрока   = Макет.ПолучитьОбласть("Строка|Данные");

	ОбластьПодвалТаблицы  = Макет.ПолучитьОбласть("ПодвалТаблицы");
	ОбластьИтогоРазница   = Макет.ПолучитьОбласть("Итого"); //ValMa
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		//ValMa
		ТабДокумент.Присоединить(ОбластьКоличествоСписаниеШапка);
		ТабДокумент.Присоединить(ОбластьЦенаСписаниеШапка);
		ТабДокумент.Присоединить(ОбластьСуммаСписаниеШапка);
		//--
		...
		//ValMa
		ТабДокумент.Присоединить(ОбластьКоличествоОприходованиеШапка);
		ТабДокумент.Присоединить(ОбластьЦенаОприходованиеШапка);
		ТабДокумент.Присоединить(ОбластьСуммаОприходованиеШапка);
		//--
		
		ТабДокумент.Присоединить(ОбластьДанныеШапка);

		ИтогоРазница = 0; //ValMa
		ИтогоСуммаСп = 0;
		ИтогоСуммаОп = 0;
		...
		Пока ВыборкаПоСтрокам.Следующий() Цикл
			...
			//ValMa
			ОбластьКоличествоСписаниеСтрока.Параметры.Заполнить(ВыборкаПоСтрокам);
			ТабДокумент.Присоединить(ОбластьКоличествоСписаниеСтрока);
			
			ОбластьЦенаСписаниеСтрока.Параметры.Заполнить(ВыборкаПоСтрокам);
			ТабДокумент.Присоединить(ОбластьЦенаСписаниеСтрока);
			
			ОбластьСуммаСписаниеСтрока.Параметры.Заполнить(ВыборкаПоСтрокам);
			ТабДокумент.Присоединить(ОбластьСуммаСписаниеСтрока);
			
			ИтогоСуммаСп = ИтогоСуммаСп + ВыборкаПоСтрокам.СуммаСписание;
			//--
			...
			//ValMa
			ОбластьКоличествоОприходованиеСтрока.Параметры.Заполнить(ВыборкаПоСтрокам);
			ТабДокумент.Присоединить(ОбластьКоличествоОприходованиеСтрока);
			
			ОбластьЦенаОприходованиеСтрока.Параметры.Заполнить(ВыборкаПоСтрокам);
			ТабДокумент.Присоединить(ОбластьЦенаОприходованиеСтрока);
			
			ОбластьСуммаОприходованиеСтрока.Параметры.Заполнить(ВыборкаПоСтрокам);
			ТабДокумент.Присоединить(ОбластьСуммаОприходованиеСтрока);
			
			ИтогоСуммаОп = ИтогоСуммаОп + ВыборкаПоСтрокам.СуммаОприходование;
			//--
			
			// Данные о разнице сумм.
			...
			ИтогоРазница = ИтогоРазница + ВыборкаПоСтрокам.РазницаСумм; //ValMa
		...
		//ValMa
		ОбластьИтогоРазница.Параметры.ИтогоСуммаСписание      = ИтогоСуммаСп;
		ОбластьИтогоРазница.Параметры.ИтогоСуммаОприходование = ИтогоСуммаОп;
		ОбластьИтогоРазница.Параметры.ИтогоРазница            = ИтогоРазница;
		ТабДокумент.Вывести(ОбластьИтогоРазница);
		//--
		...
		//ValMa
		ТекстИтоговойСтроки = НСтр("ru = 'Всего наименований %ВсегоНаименований%, %НедостачаПревышение% на сумму %ИтогоРазница% руб.'"); //ValMa
		
		ТекстИтоговойСтроки = СтрЗаменить(ТекстИтоговойСтроки,"%ВсегоНаименований%", ВсегоНаименований);
		Если ИтогоРазница<0 Тогда
			ТекстИтоговойСтроки = СтрЗаменить(ТекстИтоговойСтроки,"%НедостачаПревышение%", "недостача");
		ИначеЕсли ИтогоРазница>0 Тогда
			ТекстИтоговойСтроки = СтрЗаменить(ТекстИтоговойСтроки,"%НедостачаПревышение%", "превышение");
		КонецЕсли;
		МодульРазницы = ?(ИтогоРазница > 0, ИтогоРазница, -ИтогоРазница);
		ТекстИтоговойСтроки = СтрЗаменить(ТекстИтоговойСтроки,"%ИтогоРазница%", Формат(МодульРазницы,"ЧДЦ=2"));
		//---
