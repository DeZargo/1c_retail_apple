	...
	Иначе
		Для Каждого СтрокаТЧ Из ТаблицаТоваровЧека Цикл
			...
			Иначе
				...
				Если СтрокаТЧ.ОбувнаяПродукция И ЗначениеЗаполнено(СтрокаТЧ.КодМаркировки) Тогда
					...
					КонецЕсли;
				ИначеЕсли СтрокаТЧ.ТабачнаяПродукция И ЗначениеЗаполнено(СтрокаТЧ.Штрихкод) Тогда 
			...
			Если НуженПересчетСуммыНДС Тогда
				Если СтрокаТЧ.НДСПоСтрочно Тогда
					ТекПроцентНДС = ОбработкаТабличнойЧастиТоварыКлиентСерверПовтИсп.ПолучитьСтавкуНДСЧислом(СтрокаТЧ.СтавкаНДС);
					СтрокаПозицииЧека.СуммаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДСПоСтрочно(СтрокаПозицииЧека.Сумма, СтрокаТЧ.КоличествоУпаковок, ТекПроцентНДС);
				Иначе
			...
			Если СтрокаТЧ.НДСПоСтрочно Тогда
				...
				МассивРазвернутыхСтрок = РазвернутьСтрокуПозицииЧека(СтрокаПозицииЧека, МассивСтрокЕГАИС);
...
Функция РазвернутьСтрокуПозицииЧека(СтрокаПозицииЧека, МассивСтрокЕГАИС)
	
	МассивРезультат = Новый Массив;
	
	Количество = СтрокаПозицииЧека.Количество;
	Цена = СтрокаПозицииЧека.Цена;
	
	Коэффициенты = Новый Массив(Количество); // Служебный массив для распределения сумм
	
	// Построчное заполнение сведений о табачной продукции
	Индекс = 0;
	
	Для Каждого СтрокаКоэффициента из Коэффициенты Цикл
		Коэффициенты[Индекс] = 1;
		
		НоваяСтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаПозицииЧека, СтрокаПозицииЧека,,"ДанныеКодаТоварнойНоменклатуры");
		
		НоваяСтрокаПозицииЧека.Количество = 1;
		
		КодАкцизнойМарки = МассивСтрокЕГАИС[Индекс].КодАкцизнойМарки;
		
		НоваяСтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.ТипМаркировки = Перечисления.ТипыМаркировкиККТ.ТабачнаяПродукция;
		НоваяСтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.ГлобальныйИдентификаторТорговойЕдиницы = Лев(КодАкцизнойМарки, 14);
		НоваяСтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.СерийныйНомер = Сред(КодАкцизнойМарки, 15, 7);
...
Функция ОбработатьДанныеШтрихкодаЕдиницыАлкогольнойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	СтруктураПоиска.Вставить("КодАкцизнойМарки", ДанныеШтрихкода.Штрихкод);
	СтрокиМарок = Объект.АкцизныеМарки.НайтиСтроки(СтруктураПоиска);
	...
	СтрокиТовара = Объект.Товары.НайтиСтроки(ПараметрыПоиска);
	
	Если СтрокиТовара.Количество() = 0 Тогда
		СтрокаТовара = ШтрихкодированиеИСПереопределяемый.СтруктураДляДобавленияСтроки();
		ЗаполнитьЗначенияСвойств(СтрокаТовара, ДанныеШтрихкода);
...
Функция ОбработатьДанныеШтрихкодаЕдиницыТабачнойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено)
	...
	Штрихкод = ДанныеШтрихкода.ШтрихкодУпаковки.ЗначениеШтрихкода;
	СтруктураПоискаМарки = Новый Структура;
	СтруктураПоискаМарки.Вставить("КодАкцизнойМарки", Штрихкод);
	...
	Иначе
		...
		Результат.ИзмененныеСтроки.Добавить(СтрокаТовара);
	КонецЕсли; 
	СтрокаТовара.Количество = СтрокаТовара.Количество + ДанныеШтрихкода.КоличествоПачек;
	СтрокаТовара.Штрихкод   = ДанныеШтрихкода.ШтрихкодУпаковки.ЗначениеШтрихкода;
