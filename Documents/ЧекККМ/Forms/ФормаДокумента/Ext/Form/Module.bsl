...
Функция ДобавитьНайденныеПозицииТоваров(СтруктураПараметров) Экспорт 
	...
	Иначе
		Если СтруктураПараметров.Номенклатура.ОбувнаяПродукция Тогда 
			СтруктураПараметров.Вставить("СворачиватьПоСтрокеПоиска", Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Если Результат <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если Результат.Свойство("АдресВХранилище") Тогда
				...
				Иначе
					РезультатЗаполнения = ЗаполнитьМаркиНаСервере(Результат.АдресВХранилище, 
											ДополнительныеПараметры, 
											СтрокаТЧ.Номенклатура, 
											СтрокаТЧ.Характеристика);
					
					Если РезультатЗаполнения.КоличествоУпаковок <> СтрокаТЧ.Количество Тогда
						СтрокаТЧ.КоличествоУпаковок = РезультатЗаполнения.КоличествоУпаковок;
						СтрокаТЧ.Упаковка           = ПредопределенноеЗначение("Справочник.УпаковкиНоменклатуры.ПустаяСсылка");
						СтрокаТЧ.Количество         = 0;
						СтрокаТЧ.Штрихкод           = РезультатЗаполнения.Штрихкод;
...
Процедура ПриЗакрытииФормыПроверкиИПодбора(Результат, ДополнительныеПараметры) Экспорт
	...
	ДанныеПоМаркам = Неопределено;
	Если ЭтоАдресВременногоХранилища(Результат) Тогда 
		ДанныеПоМаркам = Новый Структура("АдресВХранилище", Результат);
	ИначеЕсли Не Результат = Неопределено Тогда 
		ДанныеПоМаркам = Результат.Штрихкод;
	КонецЕсли;
	
	ОповещениеОткрытьФормуВводаАкцизнойМарки(ДанныеПоМаркам, ДополнительныеПараметры);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьКолонкиМаркировки = УчетАлкогольнойПродукции 
								Или ВестиУчетТабачнойПродукцииМОТП 
...
Процедура ПриИзмененииКассаККМСервер()
	...
	Если ИзменилсяМагазин Тогда
		...
		Если НЕ Объект.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
			...
			Объект.ЦенаВключаетНДС = ОбщегоНазначенияРТ.ПолучитьЗначениеРеквизитаВПривилегированномРежиме(
...
Процедура ПечатьЧека_ПослеОшибкиПечатиЧека(ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЕстьАлкогольнаяПродукцияЕГАИС Тогда
		...
		ИнтеграцияЕГАИССлужебныйКлиент.ОбработатьОчередьПередачиДанных(
			Новый ОписаниеОповещения("ПриОтменеОперации", ИнтеграцияЕГАИСРТКлиент, Объект.Ссылка),
...
Процедура ПроверитьВозможностьРазныхНалоговыхРежимовСкладов()
	...
	|ИЗ
	|	РегистрСведений.ПрименениеСистемНалогообложения.СрезПоследних(&Дата, Магазин = &Магазин) КАК ПрименениеСистемНалогообложенияСрезПоследних
	|
	|СГРУППИРОВАТЬ ПО
	|	ПрименениеСистемНалогообложенияСрезПоследних.СистемаНалогообложения";
...
&НаСервере
Процедура ЗаполнитьСистемуНалогооблажения()
	
	Элементы.СистемаНалогообложения.Видимость = ВозможныРазныеНалоговыеРежимыСкладов;
...
Процедура ВвестиАкцизнуюМарку(ИдентификаторСтроки, СтрокаТовара, ТекущееПоле = "КоличествоУпаковок")
	...
	Если ТабачнаяПродукция Тогда 
		ВвестиТабачнуюПродукцию(ТекущееПоле, СтрокаТовара);
	Иначе
		...
		ОрганизацияЕГАИС = ИнтеграцияЕГАИСРТВызовСервера.ОрганизацияЕГАИСПоОрганизацииИКассеККМ(Объект.Организация, Объект.КассаККМ);
...
&НаКлиенте
Процедура ВвестиТабачнуюПродукцию(ТекущееПоле, ТекущиеДанные)
...
Функция ПараметрыОткрытияФормыПроверкиИПодбора()
	...
	ОписаниеОповещенияПриЗакрытии = Новый ОписаниеОповещения("ПриЗакрытииФормыПроверкиИПодбора", ЭтотОбъект, ДополнительныеПараметры);
	...
	ПараметрыОткрытия.ОписаниеОповещенияПриЗакрытии = ОписаниеОповещенияПриЗакрытии;
